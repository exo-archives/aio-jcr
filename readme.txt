JCR 1.10.10 release notes	
========================

This is a Enterprise Edition version of 1.10 JCR. 
                                                               
Features of this version:
* Support deleting node that contains accented character using FTP

Changes of 1.10.10
=====================

Bug
    * [COR-209] - In LDAPService, InitialContext is not safely closed in authenticate method
    * [WS-251] - cometd service is broken with clustering:NPE because random does not seem initialized
    * [WS-252] - Incorrect core version
    * [WS-255] - redirect-on-error property is not taken in account
    * [PC-514] - Problem when rendering parameters to struts portlet
    * [PC-515] - Artifact groupID's changes causing tck-tests failing; (component applications/tck-tests)
    * [PC-516] - Several TCK test on PC fail
    * [JCR-1420] - Cannot delete node that contains accented character using FTP
    * [JCR-1421] - Data corrupt after restore a node which has been imported with version history
    * [JCR-1425] - Impossible to retrieve node property from JCR using RMI connection
    * [JCR-1434] - Problem with webdav, https and apache
    * [JCR-1437] - Determine property type from nodetype definition in DocumentViewImport for version history.
    * [JCR-1438] - Problem with JCR versionning
    * [JCR-1453] - Missed nodetypes in cluster testing configuration
    * [JCR-1457] - TESTING: TestBackupManager error in jcr 1.10.10

Improvement
    * [JCR-1429] - Determine if property is multi or single value from nodetype definition in import.

Task
    * [JCR-1439] - Automate OpenOffice plugin deploying.


Changes log of v.1.10.9:
 
Core
=========
Bug
    * [COR-207] - Can not search user with keyword that contain special character
 
Task
    * [COR-194] - component/web/css isn't in the reactor
 
WS
=========
Task
    * [WS-249] - Create a MediaTypeHelper class to be able to compare mimetypes.
 
JCR
=========
Bug
    * [JCR-1377] - JDBC Statements left open : Use of Datasources instead of DBCP and C3P0 pools
    * [JCR-1381] - NPE in class org.exoplatform.services.jcr.ext.replication.ChannelManager
    * [JCR-1398] - Lucene threads blocked when uploading big text files in WebDAV
    * [JCR-1401] - OutOfMemoryError in class org.exoplatform.services.jcr.ext.replication.ChannelManager
    * [JCR-1408] - max-buffer-size from configuration should be use to TransientValueData in import (docview and sysview).
    * [JCR-1411] - java.util.ConcurrentModificationException on org.exoplatform.services.jcr.ext.hierarchy.impl.NewUserListener.processUserStructure
 
Improvement
    * [JCR-1390] - Provide LOG information that indicates that a Node Import and its Indexing was successfully done.
    * [JCR-1402] - Provide a mechanism to manage Cache-Control header value for different mime-types from server configuration.
 
PC
=========
Bug
    * [PC-510] - Fix Kernel and Core versions in http://svn.exoplatform.org/projects/portlet-container/branches/2.0.x/pom.xml
 

Changes log of v.1.10.8:

Foundation Core
Bug
   * [COR-203] - Problem in user search with MySql and PostgresDB
Improvement
   * [COR-193] - Expose listeners in OrganizationService
Task
   * [COR-186] - impossible to change user password


JCR
Bug
   * [JCR-1337] - TESTING: Functional testing (jcr.ext, replication
mode proxy) ReplicationVersionRestoreTest - problems
   * [JCR-1338] - TESTING: Functional testing (jcr.ext, backup)
TestBackupManager - problems
   * [JCR-1357] - Upload of a file with special characters like " ' "
in filename is not supported by the FTPservice
   * [JCR-1360] - Problem with export forum nodetype in company
website after upgrate to jcr 1.10.6
   * [JCR-1366] - Nodes Version_History are not replicated to cluster nodes
   * [JCR-1367] - Problems with weekly performance testing - Session
Registry goes longer
   * [JCR-1370] - Remove files after a failed synchronization

Portlet Container
Bug
   * [PC-506] - About changing wrong groupId in POMs


Changes log of v.1.10.7:

Bug
    * [JCR-1275] - The "*" membership type is created automatically
    * [JCR-1286] - repoload build fails in JCR 1.10.x with new parent pom v6
    * [JCR-1294] - system/document import fails with formated XML
    * [JCR-1302] - JCR 1.10.6 doesn't work on PostgreSQL 8.2 and lower
    * [JCR-1319] - NullPointerException in ThreadLocalSessionProviderService
    * [JCR-1320] - SessionDataManager.getReferences can fail in some cases
    * [JCR-1328] - JCR WebDAV implementation of MKCOL command doesn't test the existance of a node with the same name before creating
    * [JCR-1332] - The temp file (cl_*.tmp) is not deleted after using in WorkspaceDataTransmiter if ChangesLogs for less than 16K.
    * [JCR-1336] - problem import export data
    * [JCR-1352] - JCR Schema initialization fails with MySQL
    * [JCR-1355] - exception when using "*" in SQL Query
    * [JCR-1363] - Problems with manual testing - tomcat-server on ftp -.IndexOutOfBoundsException

Improvement
    * [JCR-1322] - Repository repository is OFFLINE. while indexing content
    * [JCR-1349] - Listing the directory in TreeFile may return null during race condition, causing NPE.

Task
    * [JCR-1306] - Backporting EXOJCR-221 and EXOJCR-302 to the branch JCR 1.10.x
    * [JCR-1315] - Avoid scrollable JDBC ResultSet using in DataContainer
    * [JCR-1321] - Port Repository initialization for TCK execution from 1.12.x
...
Bug
    ...
    * [JCR-1117] - Strange behaviour of Permission
...
JCR-931 - WorkspaceDataTransmitter creates some temporary files in location that is not configurable
JCR-975 - Correct node ACL initialization after import
JCR-990 - Problem after importing document with special permissions
...
Tasks:
JCR-977 - Backport of Export/Import version history to 1.10.x
JCR-989 - Backport of External Value Storages support of 2GiB Values



JCR 1.10.3 changes log:
...
JOnAS-4.8.5
  ...
  1. Configuration
    ...
    * Copy exo-jcr-ear.ear into %jonas_home%/apps/autoload
...
Since JCR 1.8 version auto-update option is disabled by default (update-storage='false' in workspace configuration).
