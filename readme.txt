eXo JCR core 1.8.2 relese notes
===============================

JCR 1.8.2 changes after the v.1.8.1
...
- Correct calculating of same name siblings during workspace.copy(), workspace.clone(), node.update().
- XML Import Ancestor to save after replacing existed node (to have saved all nodes not only destination in case of IMPORT_UUID_COLLISION_REPLACE_EXISTING).
- IBM DB2 v.8 multi-index database script index name fixed.
