eXo JCR core 1.8.1 relese notes
===============================

JCR 1.8.1 changes after the v.1.8

- Anonymous user logic fixed for access validation. Anonymous user can read item if has a read access permission for ANY users. 
  In other cases anonymous user have no privileges.
- Test benachmark new tests for JCR API. Benchmark core improvements for multithread environment.
- XML DocumentView Import NullPointerException fixed on nodetype search.
- Editable ValueData tested with multiple sessions.
- Query NodeIterator will pool aquired Nodes in session pool.
