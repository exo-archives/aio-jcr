...
public class testReadFile extends BaseStandaloneTest {
  ...
  public void testReadSmallFile() throws Exception {
    ...
    File testFileSmall = createBLOBTempFile(this.getClass().getSimpleName() + "_", 1); // 1 Kb
   ...
   * <p>
   * You can put any large file in /test/resources and rename it to largefile.mp3. after that -
   * uncomment this test.
   * 
   * @throws Exception
   */
  /*
   * public void testReadLargeFile() throws Exception { File tf = new
   * File("src/test/resources/largefile.mp3"); assertTrue(tf.exists()); FileInputStream str = new
   * FileInputStream(tf); Session s = null; Credentials credentials = new CredentialsImpl("admin",
   * "admin" .toCharArray()); s = (SessionImpl) (repositoryService.getDefaultRepository()).login(
   * credentials, "ws"); s.refresh(false); Node root = s.getRootNode(); String filename =
   * "largefile.mp3"; assertFalse(s.itemExists("/" + filename)); // create nt:file node Node
   * createdNodeRef = root.addNode(filename, "nt:file"); Node dataNode =
   * createdNodeRef.addNode("jcr:content", "nt:resource"); MimeTypeResolver mimetypeResolver = new
   * MimeTypeResolver(); mimetypeResolver.setDefaultMimeType("application/zip"); String mimeType =
   * mimetypeResolver.getMimeType(filename); dataNode.setProperty("jcr:mimeType", mimeType);
   * dataNode.setProperty("jcr:lastModified", Calendar.getInstance());
   * dataNode.setProperty("jcr:data", str); str.close(); // done s.save(); SmbFile file = new
   * SmbFile("smb://" + servername + "/ws/" + filename); assertTrue(file.exists()); InputStream
   * smbis = file.getInputStream(); int smbsize = file.getContentLength(); byte[] smbbuf = new
   * byte[smbsize]; int temp1 = smbis.read(smbbuf); smbis.close(); // read length control
   * assertEquals(smbsize, temp1); logger.debug("smbfile length : " + smbsize); InputStream
   * jcrstream = dataNode.getProperty("jcr:data").getStream(); long length =
   * dataNode.getProperty("jcr:data").getLength(); byte[] nbuf = new byte[(int) length]; long temp2
   * = jcrstream.read(nbuf); jcrstream.close(); // read length control assertEquals(length, temp2);
   * logger.debug("jcr:data length : " + length); // check if received data from node via jcr
   * methods and smb equals assertTrue(java.util.Arrays.equals(smbbuf, nbuf)); // delete test
   * objects ((Node) s.getItem("/" + filename)).remove(); s.save(); assertFalse(s.itemExists("/" +
   * filename)); }
   */
