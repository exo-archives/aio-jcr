...
public abstract class BaseStandaloneTest extends TestCase {
  ...
  public static String            confURL           = "conf/standalone/test-configuration.xml";
  ...
  public void setUp() throws Exception {
    ...
    if ((repositoryService == null) || (serv == null)) {
      ...
      URL configurationURL = Thread.currentThread().getContextClassLoader().getResource(confURL);
      if (configurationURL == null)
        throw new Exception("No configuration found. Check that \"" + confURL + "\" exists !");
      ...
      URL loginURL = Thread.currentThread().getContextClassLoader().getResource(confAuth);

      if (loginURL == null)
        throw new Exception("No login config found. Check that resource " + confAuth + " exists !");

      if (System.getProperty("java.security.auth.login.config") == null)
        System.setProperty("java.security.auth.login.config", loginURL.toString());

      serv = (CIFSServiceImpl) container.getComponentInstanceOfType(CIFSServiceImpl.class);
  ...
  protected void compareStream(InputStream etalon,
                               ...
                               long etalonPos,
                               long dataPos,
                               long length) throws IOException, Exception {
    ...
    while ((eread = etalon.read(ebuff)) > 0) {
      ...
      for (int i = 0; i < eread; i++) {
        ...
        byte db = dbuff[i];
        if (eb != db)
          throw new Exception("Streams is not equals. Wrong byte stored at position " + index
