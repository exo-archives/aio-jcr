...
public abstract class SessionSocketHandler implements Runnable {
	...
	protected static Log logger = ExoLogger.getLogger("org.exoplatform.services.CIFS.smb.server.SessionSocketHandler");
	 ...
	 *            boolean
	 */
	public SessionSocketHandler(String name, SMBServer srv, int port,
	...
	public void initialize() throws Exception {
		...
		// Check if the server should bind to a particular local address, or all
		// local addresses
		...
		if (logger.isDebugEnabled() && hasDebug())
			logger.debug("Binding "
					+ getName()
					+ " session handler to local address : "
					+ (hasBindAddress() ? getBindAddress().getHostAddress()
							: "ALL"));
	}
	...
	public String toString() {
		...
		str.append(",");
		str.append(getBindAddress() != null ? getBindAddress().getHostAddress()
				: "<All>");
		str.append(":");
		str.append(getPort());
		str.append("]");
