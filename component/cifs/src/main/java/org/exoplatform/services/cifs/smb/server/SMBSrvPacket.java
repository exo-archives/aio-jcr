...
public class SMBSrvPacket {
   ...
   * 
   * @param siz
   *          Size of SMB packet buffer to allocate.
   */
   ...
   * 
   * @param off
   *          Offset to the AndX command within the SMB packet.
   * @param reqWords
   *          Minimum number of parameter words expected.
   * @param reqBytes
   *          Minimum number of bytes expected.
   * @return boolean True if the packet passes the checks, else false.
   */
  public final boolean checkAndXPacketIsValid(int off, int reqWords,
      int reqBytes) {
  ...
  public final boolean checkPacketIsValid(int reqWords, int reqBytes) {
    ...
    if (m_smbbuf[SIGNATURE] != (byte) 0xFF || m_smbbuf[SIGNATURE + 1] != 'S'
        || m_smbbuf[SIGNATURE + 2] != 'M' || m_smbbuf[SIGNATURE + 3] != 'B')
      return false;
  ...
  public final void DumpPacket(boolean dumpAll) {
    ...
    if (true) {
      ...
      System.out.println("Byte Offset: " + getByteOffset() + ", Byte Count: "
          + getByteCount());
      ...
      if (isResponse()) {
        ...
        System.out.println(SMBErrorText.ErrorString(getErrorClass(),
            getErrorCode()));
      }
    }
   ...
   * 
   * @param out
   *          Output stream associated with the session socket.
   * @param proto
   *          Protocol type, either PROTOCOL_NETBIOS or PROTOCOL_TCPIP
   ...
   *          Offset to the AndX command.
   * @param byts
   *          Byte array containing the data to be copied to the SMB packet.
   */
   ...
   * 
   * @param off
   *          Offset to the current AndX command.
   * @param cmd
   *          Secondary SMB command code.
   */
   ...
   *          int
   */
  public final void setError(boolean longError, int ntErr, int errCode,
      int errClass) {
  ...
  /**
   * Reset the byte/parameter pointer area for packing/unpacking data items from
   * the packet, and align the buffer on an int (32bit) boundary
   */
