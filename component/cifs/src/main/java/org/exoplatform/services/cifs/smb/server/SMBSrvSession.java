...
public class SMBSrvSession extends SrvSession implements Runnable
{
    ...
    private static Log logger = ExoLogger.getLogger("org.exoplatform.services.CIFS.smb.server.SMBSrvSession");
	 ...
	 * 
	 * @param handler
	 *            Packet handler used to send/receive SMBs
	 * @param srv
	 *            Server that this session is associated with.
	 */
    ...
    /**
	 * Add a new connection to this session. Return the allocated tree id for
	 * the new connection.
	 * 
	 ...
	 *            SharedDevice
	 */
    public/*protected*/ int addConnection(SharedDevice shrDev) throws TooManyConnectionsException
    {
	 ...
	 * 
	 * @param reason
	 *            java.lang.String Reason the session is being closed.
	 */
	 ...
	 * back a NetBIOS reject and hangup the session.
	 */
    ...
    protected void procSMBNegotiate() throws SMBSrvException, IOException
    {
        ...
        if (diaIdx != -1)
        {
            ...
            {
               	logger.debug("Could not get a protocol handler for the selected SMB dialect");
        ...
        else if (m_dialect <= Dialect.LanMan2_1)
        {
            ...
            //TODO security mode must be provided by some security/autentication manager 
            ...
            m_smbPkt.setParameter(11, 8);/*8 - encrypted challange or 24 -encrypted password*/
            ...
            try
            {
        		...
        		pos = DataPacker.putString(getServer().getServerName(), buf, pos,
        				true, true);
        ...
        else if (m_dialect == Dialect.NT)
        {
            ...
            int srvCapabs = Capability.Unicode + Capability.RemoteAPIs + Capability.NTSMBs
        ...
        try
        {
            ...
            while (m_state != SMBSrvSessionState.NBHANGUP)
            {
                ...
                case SMBSrvSessionState.SMBNEGOTIATE:
                    ...
                    //TODO maybe here is reason to transfer dynamic shares create to somewhere else 
    ...
    private void mapWorkspacesAsShares(){
    	String[] wks = ( (SMBServer)getServer()).getRepository().getWorkspaceNames();
        ...
        dev.setVolumeInfo(vi);
    		// Check if current workspace already do not represented in servers SharedDeviceList 
