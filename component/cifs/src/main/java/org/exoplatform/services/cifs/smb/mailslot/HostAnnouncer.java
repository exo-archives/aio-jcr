...
/**
 * <p>
 * The host announcer class periodically broadcasts a host announcement datagram
 * to inform other Windows networking hosts of the local hosts existence and
 * capabilities.
 */
public abstract class HostAnnouncer extends Thread {
	...
	protected static final Log logger = ExoLogger.getLogger("org.alfresco.smb.protocol.mailslot.HostAnnouncer");
	...
	protected final void initHostAnnounceSMB(String name) {
		...
		int pos = MailSlot.createHostAnnouncement(data, 0, name, m_comment,
				m_srvtype, m_interval, m_updateCount++);

		// Create the mailslot SMB

		m_smbPkt.initializeMailslotSMB("\\MAILSLOT\\BROWSE", data,
				pos);
	}
			...
			try {
				...
				if (isNetworkEnabled()) {
					...
					for (int i = 0; i < m_names.numberOfStrings(); i++) {
						...
						sendAnnouncement(hostName, m_smbPkt.getBuffer(), 0,
								m_smbPkt.getLength());

						// DEBUG

						if (logger.isDebugEnabled() && hasDebug())
							logger.debug("HostAnnouncer: Announced host "
									+ hostName);
					}
				} else {
				...
				if (m_shutdown == false) {
					logger.error("HostAnnouncer error", ex);
					logger
							.error(" Check <broadcast> setting in file-servers.xml");
				}
				m_shutdown = true;
			} catch (Exception ex) {
				// Debug
	 ...
	 */
	protected abstract void sendAnnouncement(String hostName, byte[] buf,
