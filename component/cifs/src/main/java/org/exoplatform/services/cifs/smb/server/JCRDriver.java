...
public class JCRDriver {
	...
	throws FileExistsException, LockException, RepositoryException
	{
		  // logger.debug(" createFile params ["+params+"] TEMPLATE");
			...
			Node nodeRef = createNode(session, path,
					!params.isDirectory());
   ...
   /**
            ...
            throws RepositoryException
    {
      try{
        ...
        //TODO here is a sense to transfer name encoding  directly to responsemaker 
        //  (like Trans2FindFirst2, Trans2FindNext2) and encode name at last possible
        ...
        if (name != null)
        {
          ...
          if(nodeRef.getSession().getRootNode()!=nodeRef){
            ...
            while (ni.hasNext()){
              if(nodeRef==ni.nextNode()) el = i+1; 
              
              i++;
            }
            if(i>1) nname = name +"["+el+"]";
          }
          fileInfo.setFileName(NameCoder.EncodeName(nname));
        }
        
        // Read/write access TODO bind this shit with JCR permissions
	...
	public static int writeFile(SMBSrvSession m_sess, TreeConnection conn, NetworkFile netFile, byte[] buf, int dataPos, int dataLen, int offset) 
			throws Exception
	{
   ...
   * 
   */
  public static int readFile(SMBSrvSession m_sess, TreeConnection conn, NetworkFile netFile, byte[] buf, int dataPos, int maxCount, int offset)
  ...
  public static void truncateFile(SMBSrvSession m_sess, TreeConnection conn, NetworkFile netFile, long wrtoff)throws Exception {
  // Check if the file is a directory
