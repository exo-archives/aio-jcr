...
import org.exoplatform.services.cifs.netbios.win32.WinsockError;
...
import org.exoplatform.services.cifs.smb.mailslot.WinsockNetBIOSHostAnnouncer;
...
public class Win32NetBIOSSessionSocketHandler extends SessionSocketHandler
  ...
  private static final ThreadGroup THREAD_GROUP_SESSION = new ThreadGroup(
      "W32NETBIOS_SESSION_GROUP");
  ...
  // Dummy socket used to register the workstation name that some clients search
  // for, although they connect
   ...
   *          boolean
   */
  public Win32NetBIOSSessionSocketHandler(SMBServer srv, String nbName,
      boolean debug) {
    super("Win32 NetBIOS", srv, debug);
  ...
  /**
   * Return the LANA offline polling interval to check for the LANA coming back
   * online
   * 
   * @return long
   */
  ...
  public void initialize() throws Exception {
    ...
    // Enumerate the LAN adapters, use the first available if the LANA has not
    // been specified in
    // the configuration
    ...
    if (lanas != null && lanas.length > 0) {
      ...
      // Check if the LANA has been specified via the configuration, if not then
      // use the first
      // available
      ...
      if (m_lana == -1)
        ...
        // If the LANA is not available the main listener thread will poll the
        // available
  ...
  private final void initializeWinsockNetBIOS() throws Exception {
    ...
    // Create a NetBIOS socket using the workstation name, some clients search
    // for this name

    m_wksSocket = NetBIOSSocket.createListenerSocket(getLANANumber(),
        m_wksNbName);
  }
    ...
    try {
      ...
      while (hasShutdown() == false) {
        ...
        if (isLANAValid()) {
          ...
          if (isUsingWinsock()) {
            // Wait for an incoming session request using the Winsock NetBIOS
            // interface

            runWinsock();
          } else {
            // Wait for an incoming session request using the Win32 Netbios()
            // API interface
          ...
          // Check if the network adapter/LANA is back online, if so then
          // re-initialize
          ...
          if (logger.isDebugEnabled() && hasDebug() && isLANAValid())
            logger.debug("Win32 NetBIOS LANA " + getLANANumber()
                + " Back Online");
        }
      }
    } catch (Exception ex) {
  ...
  private final void runNetBIOS() throws Exception {
    ...
    if (logger.isDebugEnabled() && hasDebug())
      logger
          .debug("Waiting for Win32 NetBIOS session request (Netbios API) ...");
    ...
    int lsn = Win32NetBIOS.Listen(m_lana, m_nbName.getNetBIOSName(),
        m_acceptClient, callerNameBuf);
    ...
    if (lsn >= 0) {
      ...
      try {
        ...
        if (logger.isDebugEnabled() && hasDebug())
          logger.debug("Win32 NetBIOS session request received, lsn=" + lsn
              + ", caller=[" + callerName + "]");

        // Create a packet handler for the session

        PacketHandler pktHandler = new Win32NetBIOSPacketHandler(m_lana, lsn,
            callerName);
        ...
        if (logger.isDebugEnabled() && hasDebug())
          logger.debug("Win32 NetBIOS Failed to create session, "
              + ex.toString());
      }
    } else {
      ...
      if (sts == NetBIOS.NRC_Bridge) {
        ...
        if (logger.isDebugEnabled() && hasDebug())
          logger.debug("Win32 NetBIOS LANA offline/disabled, LANA="
              + getLANANumber());
      } else if (logger.isDebugEnabled() && hasDebug())
        logger.debug("Win32 NetBIOS Listen error, 0x"
            + Integer.toHexString(-lsn) + ", " + NetBIOS.getErrorString(-lsn));
    }
  }
  ...
  private final void runWinsock() throws Exception {
    ...
    try {
      ...
      if (ex.getErrorCode() == WinsockError.WsaENetDown) {
        ...
        if (isLANAOnline(m_lana) == false) {
          // Network/LANA is offline, cleanup the current listening sockets and
          // wait for the
    ...
    if (sessSock != null) {
      ...
      try {
        ...
        PacketHandler pktHandler = new WinsockNetBIOSPacketHandler(m_lana,
            sessSock);
        ...
        if (logger.isDebugEnabled() && hasDebug())
          logger.debug("Winsock NetBIOS Failed to create session, "
              + ex.toString());
      }
    }
  }
   ...
   *          boolean
   */
  public final static void createSessionHandlers(SMBServer server,
      boolean sockDbg) {
    ...
    if (config.getWin32LANA() != -1) {
      ...
      sessHandler = new Win32NetBIOSSessionSocketHandler(server, config
          .getWin32LANA(), sockDbg);
      ...
      try {
        ...
        if (logger.isDebugEnabled() && sockDbg) {
          logger
              .debug("Win32 NetBIOS failed to create session handler for LANA "
                  + config.getWin32LANA());
          logger.debug("      " + ex.getMessage());
        }
      }
      ...
      if (config.hasWin32EnableAnnouncer()) {
        ...
        if (config.useWinsockNetBIOS()) {
          ...
          hostAnnouncer = new WinsockNetBIOSHostAnnouncer(sessHandler, domain,
              intvl);
        } else {
          // Create a Win32 Netbios() API announcer

          hostAnnouncer = new Win32NetBIOSHostAnnouncer(sessHandler, domain,
              intvl);
        }
      ...
      if (lanas != null && lanas.length > 0) {
        ...
        for (int i = 0; i < lanas.length; i++) {
          ...
          sessHandler = new Win32NetBIOSSessionSocketHandler(server, lana,
              sockDbg);
          ...
          if (config.hasWin32EnableAnnouncer()) {
            ...
            if (config.useWinsockNetBIOS()) {
              ...
              hostAnnouncer = new WinsockNetBIOSHostAnnouncer(sessHandler,
                  domain, intvl);
            } else {
              // Create a Win32 Netbios() API announcer

              hostAnnouncer = new Win32NetBIOSHostAnnouncer(sessHandler,
                  domain, intvl);
            }
      ...
      Win32NetBIOSLanaMonitor lanaMonitor = new Win32NetBIOSLanaMonitor(server,
          lanas, LANAPollingInterval, sockDbg);
  ...
  public void lanaStatusChange(int lana, boolean online) {
    // If the LANA has gone offline, close the listening socket and wait for the
    // LANA to
    // come back online
