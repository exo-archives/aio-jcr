...
public class Win32NetBIOSSessionSocketHandler extends SessionSocketHandler
	...
	private static final Log logger = ExoLogger.getLogger("org.exoplatform.services.CIFS.smb.server.win32");
	...
	public Win32NetBIOSSessionSocketHandler(SMBServer srv, boolean debug) {
		...
		NetBIOSName accName = new NetBIOSName("*", NetBIOSName.WorkStation,
				false);
	 ...
	 *            boolean
	 */
	public Win32NetBIOSSessionSocketHandler(SMBServer srv, int lana,
			boolean debug) {
		super("Win32 NetBIOS", srv, debug);
	...
	private final void initializeWinsockNetBIOS() throws Exception {
		...
		m_nbSocket = NetBIOSSocket.createListenerSocket(getLANANumber(),
				m_nbName);
			...
			while (hasShutdown() == false) {
				...
				if (isLANAValid()) {
					...
					try {
						...
						if (logger.isDebugEnabled() && hasDebug()) {
							logger
									.debug("Win32 NetBIOS Failed To ReInitialize LANA");
	...
	private final void runNetBIOS() throws Exception {
		...
		if (lsn >= 0) {
			...
			try {
				...
				if (logger.isDebugEnabled() && hasDebug())
					logger.debug("Win32 NetBIOS session request received, lsn="
							+ lsn + ", caller=[" + callerName + "]");

				// Create a packet handler for the session

				PacketHandler pktHandler = new Win32NetBIOSPacketHandler(
						m_lana, lsn, callerName);

				// Create a server session for the new request, and set the
				// session id.

				SMBSrvSession srvSess = new SMBSrvSession(pktHandler,
						getServer());
				srvSess.setSessionId(getNextSessionId());
				srvSess.setUniqueId(pktHandler.getShortName()
				...
				srvThread.setName("Sess_W" + srvSess.getSessionId() + "_LSN"
						+ lsn);
				srvThread.start();
			} catch (Exception ex) {
	...
	private final void runWinsock() throws Exception {
		...
		if (logger.isDebugEnabled() && hasDebug())
			logger
					.debug("Waiting for Win32 NetBIOS session request (Winsock) ...");
		...
		try {
			...
			if (ex.getErrorCode() == WinsockError.WsaENetDown) {
				...
				if (isLANAOnline(m_lana) == false) {
					...
					if (logger.isDebugEnabled() && hasDebug())
						logger.debug("Winsock NetBIOS network down, LANA="
								+ m_lana);
				}
			} else {
				// Debug

				if (hasShutdown() == false && logger.isDebugEnabled()
						&& hasDebug())
					logger.debug("Winsock NetBIOS listen error, "
							+ ex.getMessage());
			}
		}
		...
		if (sessSock != null) {
			...
			try {
				...
				if (logger.isDebugEnabled() && hasDebug())
					logger
							.debug("Winsock NetBIOS session request received, caller="
									+ sessSock.getName());
