...
public class NetBIOSSessionSocketHandler extends SessionSocketHandler {
   ...
   *          boolean
   */
  public NetBIOSSessionSocketHandler(SMBServer srv, int port,
      InetAddress bindAddr, boolean debug) {
  ...
  public final static void createSessionHandlers(SMBServer server,
      boolean sockDbg) throws Exception {
    ...
    SessionSocketHandler sessHandler = new NetBIOSSessionSocketHandler(server,
        config.getNetBIOSSessionPort(), config.getSMBBindAddress(), sockDbg);
    ...
    if (logger.isDebugEnabled() && sockDbg)
      logger.debug("TCP NetBIOS session handler created on port " +
          config.getNetBIOSSessionPort());
    ...
    if (config.hasEnableAnnouncer()) {
      ...
      if (logger.isDebugEnabled() && sockDbg)
        logger.debug("TCP NetBIOS host announcer created on port " +
            config.getNetBIOSDatagramPort());
    }
  }
}
