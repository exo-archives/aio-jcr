...
class SMBSrvTransPacket extends SMBTransPacket {
  ...
  protected static final int SetupOffsetResponse = PARAMWORDS
      + (StandardParamsResponse * 2);

  /**
   * Construct an SMB transaction packet
   * 
   * @param buf
   *          Buffer that contains the SMB transaction packet.
   */
   ...
   *          Reply SMB packet.
   * @param prmCnt
   *          Count of returned parameter bytes.
   * @param prmPos
   *          Starting offset to the parameter block.
   * @param dataCnt
   *          Count of returned data bytes.
   * @param dataPos
   *          Starting offset to the data block.
   */
  public final static void initTransactReply(SMBSrvPacket pkt, int prmCnt,
      int prmPos, int dataCnt, int dataPos) {
   ...
   * is word aligned within the byte buffer area of the SMB packet. This method
   * must be called after the parameter count has been set.
   * 
   * @param prmLen
   *          Parameter block length, in bytes.
   ...
   * 
   * @param prmblk
   *          Array to unpack the parameter block words into.
   */
   ...
   * 
   * @param pos
   *          Offset to the string within the parameter block.
   * @param uni
   *          Unicode if true, else ASCII
   * @return int
   */
   ...
   */
  protected final void doTransactionResponse(SMBSrvSession sess,
      TransactBuffer tbuf) throws IOException {
    ...
    // Check if the transaction parameter block and data block will fit within a
    // single request
    // packet
    ...
    if ((plen + dlen) > availBuf) {
      ...
      // Calculate the parameter/data block sizes to send in the first request
      // packet
      ...
      if ((plen + dlen) > availBuf) {
        ...
        if (plen > 0) {
          ...
          if (plen <= availBuf) {
            ...
            // Pack all of the parameter block and fill the remaining buffer
            // with the
            // data block
