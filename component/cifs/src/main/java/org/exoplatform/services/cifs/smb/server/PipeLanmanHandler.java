...
class PipeLanmanHandler {
  ...
  public final static boolean processRequest(TransactBuffer tbuf,
    ...
    if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
      logger.debug("\\PIPE\\LANMAN\\ transact request, cmd=" + cmd + ", prm="
          + prmDesc + ", data=" + dataDesc);
    ...
    default:
      ...
      if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
        logger.debug("No handler for \\PIPE\\LANMAN\\ request, cmd=" + cmd
   ...
   * @return true if the transaction has been processed, else false.
   */
  protected final static boolean procNetShareEnum(SMBSrvSession sess,
      TransactBuffer tbuf, String prmDesc, String dataDesc,
      SMBSrvTransPacket tpkt) throws IOException, SMBSrvException {
  ...
  protected final static boolean procNetShareGetInfo(SMBSrvSession sess,
    ...
    if (infoLevel == 1 && dataDesc.compareTo("B13BWz") == 0) {
      ...
      if (share.getComments() != null)
        strPos = dataBuf.putStringAt(share.getComments(), strPos, false, true);
      else
        strPos = dataBuf.putStringAt("", strPos, false, true);
      ...
      if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
        logger.debug("NetShareGetInfo - UNSUPPORTED " + shareName
  ...
  protected final static boolean procNetWkstaGetInfo(SMBSrvSession sess,
    ...
    if ((infoLevel == 1 && dataDesc.compareTo("zzzBBzzz") == 0)
      ...
      if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
        logger.debug("NetWkstaGetInfo UNSUPPORTED infoLevel=" + infoLevel
            + ", dataDesc=" + dataDesc);
