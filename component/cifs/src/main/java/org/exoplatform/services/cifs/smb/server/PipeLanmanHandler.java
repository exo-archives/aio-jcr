...
class PipeLanmanHandler {
	 ...
	 * 
	 * @param tbuf
	 *            Transaction setup, parameter and data buffers
	 * @param sess
	 *            SMB server session that received the transaction.
	 * @param trans
	 *            Packet to use for reply
	 ...
	 */
	public final static boolean processRequest(TransactBuffer tbuf,
			SMBSrvSession sess, SMBSrvPacket trans) throws IOException,
			SMBSrvException {
		...
		if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
			logger.debug("\\PIPE\\LANMAN\\ transact request, cmd=" + cmd
		...
		case PacketType.RAPServerGetInfo:
			processed = procNetServerGetInfo(sess, tbuf, prmDesc, dataDesc,
					tpkt);
			break;
		...
		case PacketType.NetPrintQGetInfo:
			processed = procNetPrintQGetInfo(sess, tbuf, prmDesc, dataDesc,
					tpkt);
			break;
		...
		default:
			...
			if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
				logger.debug("No handler for \\PIPE\\LANMAN\\ request, cmd="
	 ...
	 * 
	 * @param sess
	 *            Server session that received the request.
	 * @param tbuf
	 *            Transaction buffer
	 * @param prmDesc
	 ...
	 */
	protected final static boolean procNetServerGetInfo(SMBSrvSession sess,
			TransactBuffer tbuf, String prmDesc, String dataDesc,
			SMBSrvTransPacket tpkt) throws IOException, SMBSrvException {

		// Validate the parameter string

		if (prmDesc.compareTo("WrLh") != 0)
			throw new SMBSrvException(SMBStatus.SRVInternalServerError,
					SMBStatus.ErrSrv);
		...
		if (infoLevel == 1 && dataDesc.compareTo("B16BBDz") == 0) {
			...
			TransactBuffer replyBuf = new TransactBuffer(tbuf.isType(), 0, 6,
					1024);
...
protected final static boolean procNetShareEnum(SMBSrvSession sess, TransactBuffer tbuf, String prmDesc,
            String dataDesc, SMBSrvTransPacket tpkt) throws IOException, SMBSrvException
    {
        ...
        if (infoLevel == 1 && dataDesc.compareTo("B13BWz") == 0)
        {
            ...
            SharedDeviceList shrList = sess.getSMBServer().getAllShares(sess);
            int shrCount = 0;
            int strPos = 0;
            ...
            while (enm.hasMoreElements())
            {
                ...
                if (shrDev.getComments() != null)
                    strPos = dataBuf.putStringAt(shrDev.getComments(), strPos, false, true);
                else
	 ...
	 * @return true if the transaction has been processed, else false.
	 */
	protected final static boolean procNetShareGetInfo(SMBSrvSession sess,
			TransactBuffer tbuf, String prmDesc, String dataDesc,
			SMBSrvTransPacket tpkt) throws IOException, SMBSrvException {
		...
		if (infoLevel == 1 && dataDesc.compareTo("B13BWz") == 0) {
			...
			try {
				...
				share = sess.getSMBServer().findShare(shareName,
						ShareType.UNKNOWN, sess, false);
			} catch (Exception ex) {
			}

			if (share == null) {
				sess.sendErrorResponseSMB(SMBStatus.SRVInternalServerError,
						SMBStatus.ErrSrv);
				return true;
			}
			...
			if (share.getComments() != null)
				strPos = dataBuf.putStringAt(share.getComments(), strPos, false,
						true);
			else
				strPos = dataBuf.putStringAt("", strPos, false, true);
			...
			if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
				logger
						.debug("NetShareGetInfo - UNSUPPORTED " + shareName
								+ ", infoLevel=" + infoLevel + ", dataDesc="
								+ dataDesc);
	 ...
	 * @return true if the transaction has been processed, else false.
	 */
	protected final static boolean procNetWkstaGetInfo(SMBSrvSession sess,
			TransactBuffer tbuf, String prmDesc, String dataDesc,
			SMBSrvTransPacket tpkt) throws IOException, SMBSrvException {
		...
		if ((infoLevel == 1 && dataDesc.compareTo("zzzBBzzz") == 0)
			...
			dataBuf.putStringPointer(strPos);
			strPos = dataBuf.putStringAt(sess.getServerName(), strPos, false,
					true);
			...
			if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
				logger.debug("NetWkstaGetInfo UNSUPPORTED infoLevel="
	 ...
	 * @return true if the transaction has been processed, else false.
	 */
	protected final static boolean procNetPrintQGetInfo(SMBSrvSession sess,
			TransactBuffer tbuf, String prmDesc, String dataDesc,
			SMBSrvTransPacket tpkt) throws IOException, SMBSrvException {
		...
		if (logger.isDebugEnabled() && sess.hasDebug(SMBSrvSession.DBG_IPC))
			logger.debug("NetPrintQGetInfo - " + shareName + ", infoLevel="
					+ infoLevel);
