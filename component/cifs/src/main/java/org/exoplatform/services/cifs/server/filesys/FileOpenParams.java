...
import org.exoplatform.services.cifs.smb.SharingMode;
...
public class FileOpenParams {
	 ...
	 *            long
	 */
	public FileOpenParams(String path,String stream ,int openAction, int accessMode,
			int searchAttr, int fileAttr, int allocSize, long createDate) {
	 ...
	 */
	public FileOpenParams(String path, int openAction, int accessMode,
			int attr, int sharedAccess, long allocSize, int createOption,
			int rootFID, int secLevel, int secFlags) {
	...
	/**
	 * Return the open/create file/directory action All actions are mapped to
	 * the FileAction.NTxxx action codes.
	 * 
	 * @return int
	 */
	...
	/**
	 * Check if a file stream has been specified in the path to be
	 * created/opened
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Determine if write-through mode is enabled (buffering is not allowed if
	 * enabled)
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Return the security impersonation level. Levels are defined in the WinNT
	 * class.
	 * 
	 * @return int
	 */
	...
	private final int convertToNTAccessMode(int accessMode) {
		...
		// Convert the Core/LanMan SMB dialect format access mode value to an NT
		// access mode
