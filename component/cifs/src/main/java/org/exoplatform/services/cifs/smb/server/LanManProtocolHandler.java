...
public class LanManProtocolHandler extends CoreProtocolHandler {
	...
	protected final int procChainedTreeConnectAndX(int cmdOff,
		...
		if (logger.isDebugEnabled() && m_sess.hasDebug(SMBSrvSession.DBG_TREE))
			logger
					.debug("ANDX Tree Connect AndX - " + uncPath + ", "
							+ service);
		...
		try {
				...
				Credentials credentials = new CredentialsImpl("admin", "admin".toCharArray());
		  	    ...
		  	    // obtain session of shared device - workspace and
		  	    	if(logger.isDebugEnabled()){
		   	    	...
		   	    	s = (SessionImpl) (this.m_sess.getSMBServer().getRepository()).login(credentials, shareDev.getName());
		  	    }
		  	    catch(Exception ex){
			...
			if (logger.isDebugEnabled()
					&& m_sess.hasDebug(SMBSrvSession.DBG_TREE))
				logger.debug("ANDX Tree Connect AndX - Allocated Tree Id = "
						+ treeId);
		} catch (TooManyConnectionsException ex) {

			// Too many connections open at the moment

			outPkt.setError(SMBStatus.SRVNoResourcesAvailable,
							SMBStatus.ErrSrv);
			return endOff;
		}
		...
		pos = DataPacker.putString(ShareType.TypeAsService(servType),
				outBuf, pos, true);
	...
	protected final void procOpenAndX(SMBSrvPacket outPkt)
		...
		if (conn == null) {
			m_sess.sendErrorResponseSMB(SMBStatus.SRVInvalidTID,
					SMBStatus.ErrSrv);
			return;
		}
		...
		if (conn.getSharedDevice().getType() == ShareType.ADMINPIPE) {
			...
			// m_sess.sendErrorResponseSMB(SMBStatus.DOSAccessDenied,
			// SMBStatus.ErrDos);
			 m_sess.sendErrorResponseSMB(SMBStatus.SRVNotSupported,
			 SMBStatus.ErrSrv);
    ...
    try{
      ...
      if(file!=null) fid = conn.addFile(file, getSession());
    ...
    logger.debug("resp Action "+FileAction.asString(responseAction)+", fid = "+fid+" TEMPORARY");
	...
	protected final void procReadAndX(SMBSrvPacket outPkt)
		...
		if (netFile == null) {
			m_sess.sendErrorResponseSMB(SMBStatus.DOSInvalidHandle,
					SMBStatus.ErrDos);
			return;
		}
		...
		try {
			...
			rdlen = JCRDriver.readFile(m_sess, conn, netFile, buf, dataPos,
					maxCount, offset);
			...
			logger
					.error("File Read Error [" + netFile.getFileId() + "] : ",
							ex);

			// Failed to read the file

			m_sess.sendErrorResponseSMB(SMBStatus.HRDReadFault,
					SMBStatus.ErrHrd);
			return;
   ...
   // logger.debug("oldName ["+oldName+"]; newName ["+newName+"] TEMPORARY");
    ...
    try{
      ...
      m_sess.sendErrorResponseSMB(SMBStatus.ERRBadPath,
          SMBStatus.ErrDos);  
      return;
    }catch(RepositoryException e){
      m_sess.sendErrorResponseSMB(SMBStatus.SRVInternalServerError,
    ...
    try{
      ...
      m_sess.sendErrorResponseSMB(SMBStatus.ERRBadFile,
          SMBStatus.ErrDos);  
      return;
    }catch(RepositoryException e){
      m_sess.sendErrorResponseSMB(SMBStatus.SRVInternalServerError,
	 ...
	 *            Response SMB packet.
	 */
	protected void procSessionSetup(SMBSrvPacket outPkt)
		...
		if (logger.isDebugEnabled()
				&& m_sess.hasDebug(SMBSrvSession.DBG_NEGOTIATE))
			logger.debug("Session setup from user=" + user + ", password="
					+ pwd + ", domain=" + domain + ", os=" + clientOS + ", VC="
					+ vcNum + ", maxBuf=" + maxBufSize + ", maxMpx=" + maxMpx);
		...
		pos = DataPacker.putString("JLAN Server "
				+ m_sess.getServer().isVersion(), buf, pos, true);
	...
	protected void procTransact2(SMBSrvPacket outPkt) throws IOException,
		...
		if (tranPkt.getTotalParameterCount() == tranPkt
			...
			transBuf.appendSetup(buf, tranPkt.getSetupOffset(), tranPkt
					.getSetupCount() * 2);
			transBuf.appendParameter(buf, tranPkt.getParameterBlockOffset(),
					tranPkt.getParameterBlockCount());
		...
		transBuf.setReturnLimits(tranPkt.getMaximumReturnSetupCount(), tranPkt
				.getMaximumReturnParameterCount(), tranPkt
				.getMaximumReturnDataCount());
	...
	protected void procTreeConnectAndX(SMBSrvPacket outPkt)
			throws SMBSrvException, TooManyConnectionsException,
			java.io.IOException {
		...
		try {
			...
			shareDev = m_sess.getSMBServer().findShare(
					share.getShareName(), servType, getSession(), true);
		}catch (Exception ex) {
	...
	protected final void procWriteAndX(SMBSrvPacket outPkt)
     ...
     logger.debug(":procWriteAndX");  
		...
		if (logger.isDebugEnabled()
				&& m_sess.hasDebug(SMBSrvSession.DBG_FILEIO))
			logger.debug("File Write AndX [" + netFile.getFileId()
					+ "] : Size=" + dataLen + " ,Pos=" + offset);
		...
		try {
			...
			logger.error("File Write Error [" + netFile.getFileId() + "] : ",
					ex);

			// Failed to read the file

			m_sess.sendErrorResponseSMB(SMBStatus.HRDWriteFault,
					SMBStatus.ErrHrd);
			return;
		}
		catch(Exception e){
	...
	public boolean runProtocol() throws java.io.IOException, SMBSrvException,
		...
		if (chainedCmd) {
			...
			if (logger.isDebugEnabled()
					&& m_sess.hasDebug(SMBSrvSession.DBG_STATE))
	...
	protected final void procTrans2FindFirst(SrvTransactBuffer tbuf,
		...
		try {
			...
			if (logger.isDebugEnabled()
				...
				logger.debug("Start trans search [" + searchId + "] - "
						+ srchPath + ", attr=0x"
						+ Integer.toHexString(srchAttr) + ", maxFiles="
			...
			while (pktDone == false && fileCnt < maxFiles) {
				...
				// Check if the file information will fit into the return buffer
				...
				else if (FindInfoPacker.calcInfoSize(info, infoLevl, false,
						true) <= maxLen) {
					...
					packLen = FindInfoPacker.packInfo(info, dataBuf, infoLevl,
							tbuf.isUnicode());
			...
			if (searchDone == true) {
				...
				if (logger.isDebugEnabled()
						&& m_sess.hasDebug(SMBSrvSession.DBG_SEARCH))
					logger.debug("End start search [" + searchId
							+ "] (Search complete)");
			...
			m_sess.sendErrorResponseSMB(SMBStatus.DOSNoMoreFiles,
					SMBStatus.ErrDos);
			// m_sess.sendErrorResponseSMB(SMBStatus.DOSFileNotFound,
			// SMBStatus.ErrDos);
		}
	...
	protected final void procTrans2FindNext(SrvTransactBuffer tbuf,
		...
		try {
			...
			if (logger.isDebugEnabled()
					&& m_sess.hasDebug(SMBSrvSession.DBG_SEARCH))
				logger.debug("Continue search [" + searchId + "] - "
						+ resumeName + ", maxFiles=" + maxFiles
						+ ", infoLevel=" + infoLevl + ", flags=0x"
						+ Integer.toHexString(srchFlag));
	...
	protected final void procTrans2QueryFileSys(SrvTransactBuffer tbuf,
		...
		try {
			...
			DataBuffer replyBuf = new DataBuffer(buf, dataPos, buf.length
					- dataPos);
			...
			case DiskInfoPacker.InfoVolume:
				...
				DiskInfoPacker.packVolumeInfo(volInfo, replyBuf, tbuf
						.isUnicode());
				break;
			...
			case DiskInfoPacker.InfoFsAttribute:
				DiskInfoPacker.packFsAttribute(0, 255, "JLAN",
						tbuf.isUnicode(), replyBuf);
				break;
			}
			...
			SMBSrvTransPacket.initTransactReply(outPkt, 0, prmPos, dataLen,
					dataPos);
			outPkt
					.setByteCount(replyBuf.getPosition()
							- outPkt.getByteOffset());
			...
			SMBSrvPacket outPkt) throws java.io.IOException, SMBSrvException 
  {
    ...
    if( path.equals("")){ 
      ...
      path = NameCoder.DecodeName(path.toString());
		...
		if (logger.isDebugEnabled() && m_sess.hasDebug(SMBSrvSession.DBG_INFO))
			logger.debug("Query Path - level = 0x"
					+ Integer.toHexString(infoLevl) + ", path = " + path);
      ...
      Node node = (Node)conn.getSession().getItem(path);
			...
			FileInfo fileInfo = JCRDriver.getFileInformation(node);// .getFileInformation(m_sess, conn, path);
			...
			int dataLen = QueryInfoPacker.packInfo(fileInfo, replyBuf,
	         infoLevl, true);
