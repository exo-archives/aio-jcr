 ...
 * 
 * <p>
 * The Win32 NetBIOS host announcer sends out the announcements using datagrams
 * sent via Winsock calls.
 */
public class WinsockNetBIOSHostAnnouncer extends HostAnnouncer {
   ...
   */
  public WinsockNetBIOSHostAnnouncer(Win32NetBIOSSessionSocketHandler handler,
  ...
  protected void sendAnnouncement(String hostName, byte[] buf, int offset,
    ...
    try {
      ...
      if (logger.isErrorEnabled())
        logger.error("Host announce error, " + ex.getMessage() + ", (LANA "
            + getLana() + ")");
    }
    ...
    if (txOK == false) {
      ...
      if (incrementErrorCount() == SendErrorCount) {
        ...
        if (logger.isErrorEnabled())
          logger.error("Marked LANA as unavailable due to send errors, (LANA "
              + getLana() + ")");
      }
    } else {
      // Clear the error count
