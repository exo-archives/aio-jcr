...
public class NetBIOSSocket {
  ...
  /**
   * Create a NetBIOS datagram socket to send out mailslot announcements on the
   * specified LANA
   * 
   * @param lana
   *          int
  ...
  public final int sendDatagram(NetBIOSName toName, byte[] buf, int off, int len)
    ...
    return Win32NetBIOS.SendSocketDatagram(getSocket(),
        toName.getNetBIOSName(), buf, off, len);
  }
