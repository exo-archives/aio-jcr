 ...
 * 
 * <p>
 * Contains the details of a Winsock NetBIOS socket that was opened using native
 * code.
 * 
 * @author GKSpencer
 */
public class NetBIOSSocket {
	 ...
	 */
	public static final NetBIOSSocket createListenerSocket(int lana,
			NetBIOSName nbName) throws WinsockNetBIOSException,
	...
	/**
	 * Create a NetBIOS datagram socket to send out mailslot announcements on
	 * the specified LANA
	 * 
	 * @param lana
	 *            int
	...
	public static final NetBIOSSocket createDatagramSocket(int lana)
		...
		if (sockPtr == 0)
			throw new NetBIOSSocketException(
					"Failed to create NetBIOS datagram socket");
	 ...
	 *            int
	 */
	private NetBIOSSocket(int lana, int sockPtr, NetBIOSName nbName,
			int socketType) {
		m_lana = lana;
		m_nbName = nbName;
		m_socket = sockPtr;
	...
	public final int write(byte[] buf, int off, int len)
		...
		if (isDatagramSocket())
			throw new WinsockNetBIOSException(
					"Write not allowed for datagram socket");
	...
	public final int read(byte[] buf, int off, int maxLen)
		...
		if (isDatagramSocket())
			throw new WinsockNetBIOSException(
					"Read not allowed for datagram socket");
	 ...
	 * @exception WinsockNetBIOSException
	 */
	public final int sendDatagram(NetBIOSName toName, byte[] buf, int off,
		...
		return Win32NetBIOS.SendSocketDatagram(getSocket(), toName
				.getNetBIOSName(), buf, off, len);
	}
	 ...
	 * @exception winsockNetBIOSException
	 */
	public final NetBIOSSocket listen() throws WinsockNetBIOSException,
			NetBIOSSocketException {
