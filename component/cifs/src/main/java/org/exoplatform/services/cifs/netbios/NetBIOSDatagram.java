...
public class NetBIOSDatagram {
  ...
  public final NetBIOSName getDestinationName() {
    ...
    if (name != null) {
      ...
      NetBIOSName nbName = new NetBIOSName(name.substring(0, 14), name
          .charAt(15), false);
  ...
  public final boolean isSMBData() {
    if (m_buf[NB_USERDATA] == (byte) 0xFF
        ...
        && m_buf[NB_USERDATA + 3] == (byte) 'B' && getLength() >= NB_MINSMBLEN)
      return true;
    return false;
  }
  ...
  /**
   * Send a datagram to the specified NetBIOS name using the global NetBIOS
   * datagram socket
   * 
   * @param dgramTyp
   *          Datagram type
   ...
   *          From NetBIOS name
   * @param fromNameTyp
   *          From NetBIOS name type.
   * @param toName
   *          To NetBIOS name
   * @param toNameType
   *          To NetBIOS name type.
   * @param userData
   *          User data buffer
   * @param userLen
   *          User data length.
   * @param userOff
   *          Offset of data within user buffer.
   * @param addr
   *          Address to send to
   * @param port
   *          Port to send to
   * @exception java.io.IOException
   *              Error occurred sending datagram
   * @exception UnknownHostException
   *              Failed to generate the broadcast mask for the network
   */
  public final void SendDatagram(int dgramTyp, String fromName,
      char fromNameType, String toName, char toNameType, byte[] userData,
      int userLen, int userOff, InetAddress addr, int port) throws IOException,
  ...
  public final void SendDatagram(int dgramTyp, String fromName,
      char fromNameType, String toName, char toNameType, byte[] userData,
      int userLen, int userOff) throws IOException, UnknownHostException {
   ...
   *              Failed to generate the broadcast mask for the network
   */
  public final void SendDatagram(int dgramTyp, String fromName, String toName,
      byte[] userData, int userLen) throws IOException, UnknownHostException {

    // Send the datagram from the standard port

    SendDatagram(dgramTyp, fromName, NetBIOSName.FileServer, toName,
        NetBIOSName.FileServer, userData, userLen, 0);
  }

  /**
   * Send a datagram to the specified NetBIOS name using the supplised datagram
   * socket.
   * 
   * @param dgramTyp
   *          Datagram type
   * @param sock
   *          Datagram socket to use to send the datagram packet.
   * @param fromName
   *          From NetBIOS name
   * @param fromNameTyp
   *          From NetBIOS name type.
   ...
   */
  public final void SendDatagram(int dgramTyp, DatagramSocket sock,
      String fromName, char fromNameType, String toName, char toNameType,
      byte[] userData, int userLen, int userOff) throws IOException {
    ...
    InetAddress destAddr = InetAddress.getByName(NetworkSettings
        .GenerateBroadcastMask(null));
    DatagramPacket dgram = new DatagramPacket(m_buf, userLen + NB_USERDATA,
        destAddr, RFCNetBIOSProtocol.DATAGRAM);
  ...
  /**
   * Send a datagram to the specified NetBIOS name using the supplied datagram
   * socket.
   * 
   * @param fromName
   *          java.lang.String
   ...
   */
  public final void SendDatagram(int dgramTyp, DatagramSocket sock,
      String fromName, String toName, byte[] userData, int userLen)
      throws IOException {

    // Send the datagram from the standard port

    SendDatagram(dgramTyp, sock, fromName, NetBIOSName.FileServer, toName,
        NetBIOSName.FileServer, userData, userLen, 0);
  }
   ...
   *          Length of user data
   * @param off
   *          Offset to start of data within buffer.
   */
