...
class SrvTransactBuffer extends TransactBuffer {
  ...
  public SrvTransactBuffer(SrvTransactBuffer tbuf) {
    ...
    setReturnLimits(tbuf.getReturnSetupLimit(), tbuf.getReturnParameterLimit(),
  ...
  public SrvTransactBuffer(NTTransPacket ntpkt) {
    ...
    if (slen > 0)
      m_setupBuf = new DataBuffer(ntpkt.getBuffer(), ntpkt.getSetupOffset(),
          slen);
    ...
    if (dlen > 0)
      m_dataBuf = new DataBuffer(ntpkt.getBuffer(), ntpkt.getDataBlockOffset(),
          dlen);
  ...
  public SrvTransactBuffer(SMBSrvTransPacket tpkt) {
    ...
    if (plen > 0)
      m_paramBuf = new DataBuffer(tpkt.getBuffer(), tpkt.getRxParameterBlock(),
          plen);
