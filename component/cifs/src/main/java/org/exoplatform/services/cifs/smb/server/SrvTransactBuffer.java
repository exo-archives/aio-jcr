 ...
 * <p>
 * Contains the parameters and data for a transaction, transaction2 or NT
 * transaction request.
 */
class SrvTransactBuffer extends TransactBuffer {
	 ...
	 * Class constructor
	 * <p>
	 * Construct a TransactBuffer using the maximum size settings from the
	 * specified transaction buffer
	 * 
	 * @param tbuf
	 *            SrvTransactBuffer
	 */
	public SrvTransactBuffer(SrvTransactBuffer tbuf) {
		super(tbuf.getReturnSetupLimit(), tbuf.getReturnParameterLimit(), tbuf
				.getReturnDataLimit());

		// Save the return limits for this transaction buffer

		setReturnLimits(tbuf.getReturnSetupLimit(), tbuf
				.getReturnParameterLimit(), tbuf.getReturnDataLimit());
	...
	public SrvTransactBuffer(NTTransPacket ntpkt) {
		...
		if (slen > 0)
			m_setupBuf = new DataBuffer(ntpkt.getBuffer(), ntpkt
					.getSetupOffset(), slen);
		...
		if (plen > 0)
			m_paramBuf = new DataBuffer(ntpkt.getBuffer(), ntpkt
					.getParameterBlockOffset(), plen);

		// Set the data block and size

		int dlen = ntpkt.getDataBlockCount();
		if (dlen > 0)
			m_dataBuf = new DataBuffer(ntpkt.getBuffer(), ntpkt
					.getDataBlockOffset(), dlen);
	...
	public SrvTransactBuffer(SMBSrvTransPacket tpkt) {
		...
		if (slen > 0)
			m_setupBuf = new DataBuffer(tpkt.getBuffer(),
					tpkt.getSetupOffset(), slen);

		// Set the parameter block and size

		int plen = tpkt.getTotalParameterCount();
		if (plen > 0)
			m_paramBuf = new DataBuffer(tpkt.getBuffer(), tpkt
					.getRxParameterBlock(), plen);

		// Set the data block and size

		int dlen = tpkt.getRxDataBlockLength();
		if (dlen > 0)
			m_dataBuf = new DataBuffer(tpkt.getBuffer(), tpkt.getRxDataBlock(),
					dlen);
