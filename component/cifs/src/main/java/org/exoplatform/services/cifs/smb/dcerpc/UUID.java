...
public class UUID {
   ...
   * "1ff70682-0a51-30e8-076d-740be8cee98b" }; // Validate and convert the UUIDs
   * for ( int i = 0; i < uuids.length; i++) { UUID u = new UUID(uuids[i]); if (
   * u.isValid()) { System.out.println("" + (i+1) + ": " + u.toString()); byte[]
   * bytes = u.getBytes(); HexDump.Dump(bytes,bytes.length, 0);
   * System.out.println("Convert to string: " + generateUUIDString(bytes)); }
   * else System.out.println("Invalid UUID: " + uuids[i]); } }
   */
}
