...
public class UUID {
	...
	public final byte[] getBytes() {
		...
		if (m_uuidBytes == null) {
			...
			try {
				...
				DataPacker.putIntelInt((int) (lval & 0xFFFFFFFF), m_uuidBytes,
						0);
	...
	public static final boolean validateUUID(String idStr) {
		...
		if (idStr.charAt(8) != '-' || idStr.charAt(13) != '-'
				|| idStr.charAt(18) != '-' || idStr.charAt(23) != '-')
			return false;
	...
	/**
	 * Write the binary UUID to the specified buffer, and optionally the UUID
	 * version
	 * 
	 * @param buf
	 *            byte[]
	 ...
	 */
	/**
	 * public final static void main(String[] args) { System.out.println("UUID
	 * Test"); System.out.println("---------"); String[] uuids = {
	 ...
	 * "1ff70682-0a51-30e8-076d-740be8cee98b" }; // Validate and convert the
	 * UUIDs for ( int i = 0; i < uuids.length; i++) { UUID u = new
	 * UUID(uuids[i]); if ( u.isValid()) { System.out.println("" + (i+1) + ": " +
	 * u.toString()); byte[] bytes = u.getBytes();
	 * HexDump.Dump(bytes,bytes.length, 0); System.out.println("Convert to
	 * string: " + generateUUIDString(bytes)); } else
