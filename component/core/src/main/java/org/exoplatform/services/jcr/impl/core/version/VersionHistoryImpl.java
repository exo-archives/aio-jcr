...
public class VersionHistoryImpl extends VersionStorageDescendantNode implements VersionHistory {
  ...
  // new impl
  public VersionHistoryImpl(NodeData data, SessionImpl session) throws PathNotFoundException,
      RepositoryException {
  ...
  public String getVersionableUUID() throws RepositoryException {
    ...
    PropertyData versionableUuid = (PropertyData) dataManager.getItemData(nodeData(),
                                                                          new QPathEntry(Constants.JCR_VERSIONABLEUUID,
                                                                                         0));
  ...
  public Version getRootVersion() throws RepositoryException {
    ...
    VersionImpl version = (VersionImpl) dataManager.getItem(nodeData(),
  ...
  public Version version(String versionName, boolean pool) throws VersionException,
    ...
    VersionImpl version = (VersionImpl) dataManager.getItem(nodeData(),
                                                            new QPathEntry(jcrVersionName.getInternalName(),
                                                                           1),
                                                            pool);
    if (version == null)
  ...
  public Version getVersionByLabel(String label) throws RepositoryException {
    ...
    if (versionData == null)
      throw new RepositoryException("There are no label '" + label + "' in the version history "
          + getPath());

    VersionImpl version = (VersionImpl) dataManager.getItemByIdentifier(versionData.getIdentifier(),
                                                                        true);

    if (version == null)
      throw new VersionException("There are no version with label '" + label
          + "' in the version history " + getPath());
  ...
  public String[] getVersionLabels() throws RepositoryException {
    ...
    for (int i = 0; i < versionLabels.size(); i++) {
      labelsStrs[i] = locationFactory.createJCRName(versionLabels.get(i).getQPath().getName())
                                     .getAsString();
    }
  ...
  protected List<String> getVersionLabelsList(Version version) throws VersionException,
                                                              RepositoryException {
    if (!isVersionBelongToThis(version))
      throw new VersionException("There are no version '" + version.getPath()
          + "' in the version history " + getPath());
  ...
  public void removeVersion(String versionName) throws ReferentialIntegrityException,
    ...
    // jcr:predecessors
    PropertyData predecessorsData = (PropertyData) dataManager.getItemData((NodeData) version.getData(),
    ...
    try {
      for (ValueData pvalue : predecessorsData.getValues()) {
        ...
        if (predecessor != null) {// V2's successor
          if (successorsData != null) {// to redirect V2's successor
            ...
            for (ValueData svalue : successorsData.getValues()) {
              predecessor.removeAddSuccessor(version.getInternalIdentifier(),
                                             new String(svalue.getAsByteArray()),
                                             changes);
            }
          } else {
          ...
          throw new RepositoryException("A predecessor (" + pidentifier + ") of the version "
          ...
          if (successor != null) {
            ...
            for (ValueData pvalue : predecessorsData.getValues()) {
              successor.removeAddPredecessor(version.getInternalIdentifier(),
                                             new String(pvalue.getAsByteArray()),
                                             changes);
            }
          } else {
  ...
  protected NodeData getVersionData(String versionName) throws VersionException,
                                                       RepositoryException {
  ...
  protected NodeData getVersionLabelsData() throws VersionException, RepositoryException {
    ...
    if (labels == null)
      throw new VersionException("Mandatory node jcr:versionLabels is not found for version history "
          + getPath());

    return labels;
  }

  protected NodeData getVersionDataByLabel(String labelName) throws VersionException,
                                                            RepositoryException {
  ...
  protected NodeData getVersionDataByIdentifier(String versionIdentifier) throws VersionException,
                                                                         RepositoryException {
  ...
  public void addVersionLabel(String versionName, String label, boolean moveLabel) throws VersionException,
    ...
    PropertyData labelData = TransientPropertyData.createPropertyData(labels,
                                                                      labelQName,
  ...
  public void addVersion(NodeData versionableNodeData, String uuid, SessionChangesLog changesLog) throws RepositoryException {
    ...
    // jcr:primaryType
    TransientPropertyData propData = TransientPropertyData.createPropertyData(versionData,
    ...
    // jcr:mixinTypes
    propData = TransientPropertyData.createPropertyData(versionData,
    ...
    propData = TransientPropertyData.createPropertyData(versionData,
                                                        ...
                                                        PropertyType.DATE,
                                                        false,
                                                        new TransientValueData(session.getTransientNodesManager()
    ...
    for (ValueData predecessorValue : predecessors) {
      ...
      VersionImpl predecessor = (VersionImpl) dataManager.getItemByIdentifier(predecessorIdentifier,
                                                                              false);
    ...
    // jcr:frozenNode
    NodeData frozenData = TransientNodeData.createNodeData(versionData,
                                                           Constants.JCR_FROZENNODE,
                                                           Constants.NT_FROZENNODE);
    changesLog.add(ItemState.createAddedState(frozenData));

    propData = TransientPropertyData.createPropertyData(frozenData,
                                                        Constants.JCR_PRIMARYTYPE,
                                                        PropertyType.NAME,
                                                        false,
