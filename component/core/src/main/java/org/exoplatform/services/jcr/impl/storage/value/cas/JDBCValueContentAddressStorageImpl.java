...
public class JDBCValueContentAddressStorageImpl implements ValueContentAddressStorage {
  ...
  public void add(String propertyId, int orderNum, String identifier) throws RecordAlreadyExistsException, VCASException {
    ...
    try {
      ...
      if (e.toString().indexOf(sqlConstraintPK) >= 0) 
   ...
   */
  public List<String> getIdentifiers(String propertyId) throws RecordNotFoundException, VCASException {
    try {
      Connection con = dataSource.getConnection();
      try {
        PreparedStatement ps = con.prepareStatement(sqlSelectRecords);
        ps.setString(1, propertyId);
        ResultSet rs = ps.executeQuery();
