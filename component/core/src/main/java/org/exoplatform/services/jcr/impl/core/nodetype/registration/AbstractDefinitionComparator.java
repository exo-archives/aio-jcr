...
public abstract class AbstractDefinitionComparator<T extends ItemDefinitionData> {
  ...
  public abstract PlainChangesLog compare(NodeTypeData registeredNodeType,
                                          T[] ancestorDefinition,
                                          T[] recipientDefinition) throws ConstraintViolationException,
                                                                  RepositoryException;
   ...
   */
  protected void init(T[] ancestorDefinition,
                      T[] recipientDefinition,
                      List<T> sameDefinitionData,
                      List<RelatedDefinition<T>> changedDefinitionData,
                      List<T> newDefinitionData,
                      List<T> removedDefinitionData) {
      ...
      for (int j = 0; j < ancestorDefinition.length; j++) {
        if (recipientDefinition[i].getName().equals(ancestorDefinition[j].getName())) {
          ...
          if (recipientDefinition[i].equals(ancestorDefinition[j]))
            ...
            RelatedDefinition<T> relatedDefinition = new RelatedDefinition<T>(ancestorDefinition[j],
