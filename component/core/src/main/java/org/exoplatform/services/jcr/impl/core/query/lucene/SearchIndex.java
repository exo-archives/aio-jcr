...
public class SearchIndex extends AbstractQueryHandler implements ItemsPersistenceListener {
    ...
    /**
     * The field for transferring a DocumentReaderService variable
     * by the SearchIndex constructor to the NodeIndexer.
     */
    ...
    //private final QueryHandlerContext context;
    ...
    public void updateNodes(Iterator remove, Iterator add)
        ...
        index.update(new AbstractIteratorDecorator(remove) {
            ...
            public Object next() {
                ...
                Document doc = null;
                try {
                    doc = createDocument(state);
                } catch (RepositoryException e) {
                    log.error("Exception while creating document for node: " +
                            state.getQPath().getAsString() + ": " + e.toString(), e);
                }
                return doc;
            }
        });
...
//                                             ItemManager itemMgr,
        ...
        QueryImpl query = new QueryImpl(session, this,
                 statement, language);
    ...
    public void onSaveItems(ItemStateChangesLog changesLog) {
      ...
      try {
        ...
        log.error("Error indexing node. "+itemState.getData().getQPath().getAsString(), e);
      } catch (IOException e) {
        log.error("Error indexing node. "+itemState.getData().getQPath().getAsString(), e);
      } catch (Throwable e) {
        e.printStackTrace();
     ...
     */
    protected SortField[] createSortFields(InternalQName[] orderProps,
                                           boolean[] orderSpecs) {
    ...
    protected Document createDocument(NodeData node)
        ...
        return NodeIndexer.createDocument(node, sysLocationFactory,
