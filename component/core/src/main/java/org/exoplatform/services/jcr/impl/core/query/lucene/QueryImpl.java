...
public class QueryImpl extends AbstractQueryImpl {
    ...
    public QueryImpl(SessionImpl session,
        ...
        this.root = QueryParser.parse(statement, language,
                session.getLocationFactory(), factory);
    ...
    public QueryResult execute(long offset, long limit) throws RepositoryException {
        ...
        if (allNodesQueryNode.equals(root)) {
            return new WorkspaceTraversalResult(session,
                    new InternalQName[] { Constants.JCR_PRIMARYTYPE, Constants.JCR_PATH, Constants.JCR_SCORE },
        ...
        return new QueryResultImpl(index, itemMgr,
                session.getLocationFactory(),session.getValueFactory(), session.getAccessManager(),session.getUserID(),
    ...
    protected InternalQName[] getSelectProperties() throws RepositoryException {
        ...
        if (selectProps.size() == 0) {
            ...
            ExtendedNodeType nt = session.getWorkspace().getNodeTypeManager().getNodeType(ntName[0]);
