...
abstract public class CQJDBCStorageConnection extends JDBCStorageConnection {
    ...
    try {
      ...
      while (resultSet.next()) {
        if (data == null) {
          ...
          NodeData nodeData = loadNodeFromTemporaryNodeData(data,
                                                            parent.getQPath(),
                                                            parent.getACL());
   ...
   * @throws IOException
   */
  private List<PropertyData> loadReferences(ResultSet resultSet) throws RepositoryException,
                                                                SQLException,
                                                                IOException {
    ...
    try {
      while (resultSet.next()) {
        ...
        if (prop == null) {
          ...
          prop = new PersistedPropertyData(identifier,
                                           qpath,
                                           getIdentifier(cpid),
                                           cversion,
                                           cptype,
                                           cpmultivalued); // null values!
      ...
      for (String id : propertyBuffer.keySet()) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          PersistedPropertyData pdata = new PersistedPropertyData(prop.getIdentifier(),
                                                                  prop.getQPath(),
   ...
   */
  protected List<AccessControlEntry> readACLPermisions(String cid,
                                                       Map<String, List<byte[]>> properties) throws SQLException,
                                                                                            IllegalACLException {
   ...
   * @throws IOException
   */
  protected PersistedNodeData loadNodeFromTemporaryNodeData(TempNodeData tempData,
                                                            QPath parentPath,
                                                            AccessControlList parentACL) throws RepositoryException,
                                                                                        SQLException,
                                                                                        IOException {
   ...
   */
  private PersistedNodeData loadNodeRecord(QPath parentPath,
                                           String cname,
                                           String cid,
                                           String cpid,
                                           int cindex,
                                           int cversion,
                                           int cnordernumb,
                                           Map<String, List<byte[]>> properties,
                                           AccessControlList parentACL) throws RepositoryException,
                                                                       SQLException {
    try {
  ...
  protected QPath traverseQPath(String cpid) throws SQLException,
    ...
    do {
      ...
      try {
        ...
        QPathEntry qpe1 = new QPathEntry(InternalQName.parse(result.getString(COLUMN_NAME)),
                                         result.getInt(COLUMN_INDEX));
        boolean isChild = caid.equals(result.getString(COLUMN_ID));
        caid = result.getString(COLUMN_PARENTID);
        if (result.next()) {
          QPathEntry qpe2 = new QPathEntry(InternalQName.parse(result.getString(COLUMN_NAME)),
                                           result.getInt(COLUMN_INDEX));
          if (isChild) {
