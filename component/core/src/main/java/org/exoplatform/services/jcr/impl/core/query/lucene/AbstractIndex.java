 ...
 * synchronization: This class is not entirely thread-safe. Certain concurrent
 * access is however allowed. Read-only access on this index using
 ...
 * time.<br/> Modifying threads must be synchronized externally in a way that
 * only one thread is using the returned IndexReader and IndexWriter instances
 * returned by {@link #getIndexReader()} and {@link #getIndexWriter()} at a
 * time.<br/> Concurrent access by <b>one</b> modifying thread and multiple
 * read-only threads is safe!
 */
