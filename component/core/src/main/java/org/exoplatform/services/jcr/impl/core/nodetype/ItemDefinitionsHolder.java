...
public class ItemDefinitionsHolder {
   ...
   */
  public NodeDefinitionImpl getChildNodeDefinition(InternalQName parentNodeType,
                                                   InternalQName childName,
                                                   InternalQName childNodeType) {

    NodeDefinitionImpl def = getNodeDefinitionFromThisOrSupertypes(parentNodeType,
                                                                   childName,
                                                                   childNodeType);
   ...
   */
  public NodeDefinitionImpl getDefaultChildNodeDefinition(List<InternalQName> parentNodeTypes,
                                                          InternalQName childName) {
   ...
   */
  public PropertyDefinitionImpl getChildPropertyDefinition(InternalQName parentNodeType,
                                                           InternalQName childName,
                                                           boolean multiValued) {
  ...
  void putDefinitions(ExtendedNodeType nodeType) {
    ...
    for (NodeDefinition nodeDef : nodeDefs) {
      ...
      for (NodeType requiredNT : nodeDefImpl.getRequiredPrimaryTypes()) {
        ...
        ChildNodeDefKey nodeDefKey = new ChildNodeDefKey(nodeTypeImpl.getQName(),
                                                         nodeDefImpl.getQName(),
                                                         requiredNodeType.getQName());
        nodeDefinitions.put(nodeDefKey, nodeDefImpl);
        if (log.isDebugEnabled()) {
          log.debug("NodeDef added: parent NT: " + nodeTypeImpl.getQName() + " child nodeName: "
              + nodeDefImpl.getQName() + " childNT: " + requiredNodeType.getQName() + " hash: "
              + nodeDefKey.hashCode());
        }
      }
      // put default node definition
      DefaultNodeDefKey defNodeDefKey = new DefaultNodeDefKey(nodeTypeImpl.getQName(),
                                                              nodeDefImpl.getQName());
    ...
    for (PropertyDefinition propDef : propDefs) {
      ...
      ChildPropertyDefKey propDefKey = new ChildPropertyDefKey(nodeTypeImpl.getQName(),
                                                               propDefImpl.getQName(),
                                                               propDefImpl.isMultiple());
      propertyDefinitions.put(propDefKey, propDefImpl);
      if (log.isDebugEnabled()) {
        log.debug("PropDef added: parent NT: " + nodeTypeImpl.getQName() + " child propName: "
            + propDefImpl.getQName() + " isMultiple: " + propDefImpl.isMultiple() + " hash: "
            + propDefKey.hashCode());
      }
    }
  }
