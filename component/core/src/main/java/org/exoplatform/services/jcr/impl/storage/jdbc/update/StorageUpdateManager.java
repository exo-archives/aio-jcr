...
public class StorageUpdateManager {
  ...
  protected static final String SQL_SELECT_JCRUUID_MULTIDB        = "select I.PATH, N.ID as NID, V.ID as VID, V.DATA from JCR_MITEM I, JCR_MNODE N, JCR_MPROPERTY P, JCR_MVALUE V "
                                                                      + "WHERE I.ID = P.ID and N.ID = P.PARENT_ID and P.ID = V.PROPERTY_ID and "
                                                                      + "I.PATH like '%"
                                                                      + Constants.JCR_UUID.getAsString()
                                                                      + "%' " + "order by V.ID";

  protected static final String SQL_SELECT_JCRUUID_SINGLEDB       = "select I.PATH, N.ID as NID, V.ID as VID, V.DATA from JCR_SITEM I, JCR_SNODE N, JCR_SPROPERTY P, JCR_SVALUE V "
                                                                      + "WHERE I.ID = P.ID and N.ID = P.PARENT_ID and P.ID = V.PROPERTY_ID and "
                                                                      + "I.PATH like '%"
                                                                      + Constants.JCR_UUID.getAsString()
                                                                      + "%' " + "order by V.ID";
  ...
  protected static final String SQL_SELECT_FROZENJCRUUID_MULTIDB  = "select I.PATH, N.ID as NID, V.ID as VID, V.DATA from JCR_MITEM I, JCR_MNODE N, JCR_MPROPERTY P, JCR_MVALUE V "
                                                                      + "WHERE I.ID = P.ID and N.ID = P.PARENT_ID and P.ID = V.PROPERTY_ID and "
                                                                      + "I.PATH like '"
                                                                      + FROZENJCRUUID
                                                                      + "' "
  ...
  protected static final String SQL_SELECT_FROZENJCRUUID_SINGLEDB = "select I.PATH, N.ID as NID, V.ID as VID, V.DATA from JCR_SITEM I, JCR_SNODE N, JCR_SPROPERTY P, JCR_SVALUE V "
                                                                      + "WHERE I.ID = P.ID and N.ID = P.PARENT_ID and P.ID = V.PROPERTY_ID and "
                                                                      + "I.PATH like '"
                                                                      + FROZENJCRUUID
                                                                      + "' "
  ...
  protected static final String SQL_SELECT_REFERENCES_MULTIDB     = "select I.PATH, V.PROPERTY_ID, V.ORDER_NUM, V.DATA"
  ...
  protected static final String SQL_SELECT_REFERENCES_SINGLEDB    = "select I.PATH, V.PROPERTY_ID, V.ORDER_NUM, V.DATA"
   ...
   */
  public static synchronized String checkVersion(String sourceName,
  ...
  private String applyUpdate(boolean updateNow) throws Exception {
    ...
    if (curVersion.startsWith(STORAGE_VERSION_1_7_0)) {
      ...
      // warn
      log.warn("UPDATE IS NOT AVAILABLE from "
          + curVersion
          + " to "
          + STORAGE_VERSION_1_7_0
        ...
        try {
          ...
          if (!jcrIdentifier.getNodeIdentifier().equals(jcrIdentifier.getJcrIdentifier())) {
            log.info("STORAGE UPDATE >>>: Property jcr:uuid have to be updated with actual value. Property: "
                ...
                + ", existed: " + jcrIdentifier.getJcrIdentifier());

            update.clearParameters();
            update.setBinaryStream(1,
                                   new ByteArrayInputStream(jcrIdentifier.getNodeIdentifier()
                                                                         .getBytes()),
                                   jcrIdentifier.getNodeIdentifier().length());
            update.setString(2, jcrIdentifier.getValueId());

            if (update.executeUpdate() != 1) {
              log.warn("STORAGE UPDATE !!!: More than one jcr:uuid property values were updated. Updated value id: "
  ...
  private void fixCopyFrozenIdentifierBug(JcrIdentifier jcrIdentifier, Connection conn) throws SQLException {
    String searchCriteria = "'" + Constants.JCR_VERSION_STORAGE_PATH.getAsString() + ":1[]"
        + jcrIdentifier.getNodeIdentifier() + "%" + Constants.JCR_FROZENUUID.getAsString() + "%' ";
