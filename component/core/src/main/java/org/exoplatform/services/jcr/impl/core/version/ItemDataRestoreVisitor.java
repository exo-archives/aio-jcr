...
public class ItemDataRestoreVisitor extends ItemDataTraversingVisitor {
  ...
  ItemDataRestoreVisitor(NodeData context,
                         InternalQName restoringName,
                         NodeData history,
  ...
  ItemDataRestoreVisitor(NodeData context,
                         InternalQName destName,
                         NodeData history,
                         SessionImpl userSession,
                         boolean removeExisting,
                         SessionChangesLog delegatedChanges) throws RepositoryException {
  ...
  protected void initRestoreRoot(NodeData parentData, InternalQName name, NodeData frozen) throws RepositoryException {
    ...
    PropertyData frozenIdentifier = (PropertyData) dataManager.getItemData(frozen,
                                                                           new QPathEntry(Constants.JCR_FROZENUUID,
                                                                                          1));
    ...
    try {
        ...
        if (sameIdentifierNode != null) {
          ...
          if (sameIdentifierPath.makeParentPath().equals(nodePath.makeParentPath()) && // same
            ...
            if (sameIdentifierPath.getIndex() != nodePath.getIndex())
              ...
              nodePath = QPath.makeChildPath(parentData.getQPath(),
                                             name,
            ...
            if (removeExisting) {
              ...
              throw new ItemExistsException("Item with the same UUID as restored node "
                  + nodePath.getAsString()
                  + " already exists and removeExisting=false. Existed "
                  + userSession.getLocationFactory()
                               .createJCRPath(sameIdentifierPath)
                               .getAsString(false) + " " + sameIdentifierNode.getIdentifier());
            }
          }
        }
      }
      ...
      throw new RepositoryException("jcr:frozenUuid, error of data read "
          + userSession.getLocationFactory()
                       .createJCRPath(frozenIdentifier.getQPath())
                       .getAsString(false), e);
    } catch (IOException e) {
      throw new RepositoryException("jcr:frozenUuid, error of data read "
          + userSession.getLocationFactory()
    ...
    PropertyData frozenPrimaryType = (PropertyData) dataManager.getItemData(frozen,
                                                                            new QPathEntry(Constants.JCR_FROZENPRIMARYTYPE,
                                                                                           0));

    PropertyData frozenMixinTypes = (PropertyData) dataManager.getItemData(frozen,
                                                                           new QPathEntry(Constants.JCR_FROZENMIXINTYPES,
                                                                                          0));
    ...
    if (frozenMixinTypes != null) {
      try {
        ...
        throw new RepositoryException("jcr:frozenMixinTypes, error of data read "
            + userSession.getLocationFactory()
                         .createJCRPath(frozenMixinTypes.getQPath())
                         .getAsString(false), e);
      } catch (IllegalStateException e) {
        throw new RepositoryException("jcr:frozenMixinTypes, error of data read "
            + userSession.getLocationFactory()
    ...
    try {
      ...
      throw new RepositoryException("jcr:frozenPrimaryType, error of data read "
          + userSession.getLocationFactory()
                       .createJCRPath(frozenPrimaryType.getQPath())
                       .getAsString(false), e);
    } catch (IllegalStateException e) {
      throw new RepositoryException("jcr:frozenPrimaryType, error of data read "
          + userSession.getLocationFactory()
  ...
  protected void entering(NodeData frozen, int level) throws RepositoryException {
    ...
    if (qname.equals(Constants.JCR_FROZENNODE) && level == 0) {
      ...
      try {
        ...
        String vhIdentifier = new String(((PropertyData) dataManager.getItemData(frozen,
                                                                                 new QPathEntry(Constants.JCR_CHILDVERSIONHISTORY,
                                                                                                0))).getValues()
                                                                                                    .get(0)
                                                                                                    .getAsByteArray());
      ...
      try {
        versionableIdentifier = new String(((PropertyData) dataManager.getItemData(childHistory,
      ...
      if (versionable != null) {
        ...
        if (!removeExisting) {
          throw new ItemExistsException("Item with the same UUID "
              ...
              + userSession.getLocationFactory()
                           .createJCRPath(versionable.getQPath())
                           .getAsString(false) + " already exists and removeExisting=false");
        }
        ...
        NodeData cvFrozen = (NodeData) dataManager.getItemData(lastVersionData,
                                                               new QPathEntry(Constants.JCR_FROZENNODE,
                                                                              1));

        ItemDataRestoreVisitor restoreVisitor = new ItemDataRestoreVisitor(currentNode(),
                                                                           qname,
                                                                           childHistory,
                                                                           userSession,
                                                                           removeExisting,
      ...
      if (action == OnParentVersionAction.COPY || action == OnParentVersionAction.VERSION) {
        ...
        if (ntManager.isNodeType(Constants.MIX_REFERENCEABLE,
          ...
          try {
            jcrUuid = new String(((PropertyData) dataManager.getItemData(frozen,
                                                                         new QPathEntry(Constants.JCR_UUID,
                                                                                        0))).getValues()
                                                                                            .get(0)
                                                                                            .getAsByteArray());
          ...
          existing = (NodeData) dataManager.getItemData(currentNode(),
                                                        new QPathEntry(frozen.getQPath().getName(),
                                                                       frozen.getQPath().getIndex()));
          if (existing != null) {
          ...
          if (existingDelegared != null) {
            ...
            if (removeExisting) {
              ...
              throw new ItemExistsException("Node with the same UUID as restored child node "
                  ...
                  + " already exists and removeExisting=false. Existed "
                  + userSession.getLocationFactory()
                               .createJCRPath(existing.getQPath())
                               .getAsString(false) + " " + existing.getIdentifier());
            }
          }
        }
        ...
        // current C in the workspace will be left unchanged,
        NodeData existed = (NodeData) dataManager.getItemData(currentNode(),
                                                              new QPathEntry(frozen.getQPath()
                                                                                   .getName(), 0));
    ...
    if (currentNode() != null) {
      ...
      if (log.isDebugEnabled()) {
        log.debug("Visit property " + property.getQPath().getAsString() + " "
            + currentNode().getQPath().getAsString() + " "
            + OnParentVersionAction.nameFromValue(action));
      }
      ...
      if (action == OnParentVersionAction.COPY || action == OnParentVersionAction.VERSION
        ...
        if (qname.equals(Constants.JCR_PREDECESSORS)) {
          tagetProperty = TransientPropertyData.createPropertyData(currentNode(),
                                                                   qname,
                                                                   property.getType(),
  ...
  protected void leaving(NodeData frozen, int level) throws RepositoryException {
    ...
    if (qname.equals(Constants.JCR_FROZENNODE) && level == 0) {
      ...
      PropertyData baseVersion = TransientPropertyData.createPropertyData(restored,
                                                                          Constants.JCR_BASEVERSION,
                                                                          PropertyType.REFERENCE,
                                                                          false,
                                                                          new TransientValueData(frozen.getParentIdentifier()));

      PropertyData isCheckedOut = TransientPropertyData.createPropertyData(restored,
