...
public class JCRSQLQueryBuilder implements JCRSQLParserVisitor {
   ...
   */
  private static Map             parsers         = new ReferenceMap(ReferenceMap.WEAK,
                                                                    ReferenceMap.WEAK);
  ...
  public Object visit(ASTQuery node, Object data) {
    ...
    if (nodeTypeName != null) {
      ...
      LocationStepQueryNode[] steps = pathNode.getPathSteps();
      NodeTypeQueryNode nodeType = factory.createNodeTypeQueryNode(steps[steps.length - 1],
                                                                   nodeTypeName);
    ...
    try {
      ...
      if (type == QueryConstants.OPERATION_BETWEEN) {
        ...
        RelationQueryNode rel = createRelationQueryNode(between,
                                                        identifier,
                                                        QueryConstants.OPERATION_GE_GENERAL,
        ...
        if (node.getEscapeString() != null) {
          if (node.getEscapeString().length() == 1) {
            ...
            pattern.setValue(translateEscaping(pattern.getValue(),
                                               node.getEscapeString().charAt(0),
                                               '\\'));
          } else {
        ...
        for (int i = 1; i < node.children.length; i++) {
          RelationQueryNode rel = createRelationQueryNode(in,
                                                          identifier,
                                                          QueryConstants.OPERATION_EQ_VALUE,
  ...
  public Object visit(ASTLowerFunction node, Object data) {
    ...
    parent.addOperand(factory.createPropertyFunctionQueryNode(parent,
  ...
  private void createPathQuery(String path, int operation) {
    MergingPathQueryNode pathNode = new MergingPathQueryNode(operation,
                                                             factory.createPathQueryNode(null)
   ...
   * <code>n1</code> can be merged with another node <code>n2</code> in the following case:
   * <p/>
   ...
   * position <code>X+1</code> and the descending flag set.
   * <p/>
  ...
  private static class MergingPathQueryNode extends PathQueryNode {
    ...
    /**
     * Merges two nodes into a node which selects any child nodes of a given node.
     * <p/>
     * Example:<br/>
     * The path patterns:<br/>
     * <code>/foo/%</code> AND NOT <code>/foo/%/%</code><br/>
    ...
    /**
     * Merges two nodes into one node selecting a node on the descendant-or-self axis.
     * <p/>
     * Example:<br/>
     * The following path patterns:<br/>
     * <code>/foo/%/bar</code> OR <code>/foo/bar</code><br/>
