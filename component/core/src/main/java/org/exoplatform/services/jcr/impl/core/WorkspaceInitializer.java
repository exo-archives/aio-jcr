 ...
 * The class responsible for workspace storage initialization including:
 * - root node - for all workspaces
 * - /jcr:system - for system workspace
 *    /exo:namespaces
 *    /jcr:nodetypes
 *    /jcr:versionStorage
 * - search index root if configured
...
public class WorkspaceInitializer {
  ...
  public NodeData initWorkspace(InternalQName rootNodeType)
    ...
    if(log.isDebugEnabled())
      log.debug("Root node for " + workspaceName + " initialized. NodeType: "
        + rootNodeType + " system workspace: " + systemWorkspaceName);
  ...
  private NodeData initRootNode(InternalQName rootNodeType)
      throws RepositoryException {
    
    PlainChangesLog changesLog = new PlainChangesLogImpl();

    TransientNodeData nodeData = new TransientNodeData(Constants.ROOT_PATH,
        Constants.ROOT_UUID, -1, rootNodeType, new InternalQName[0], 0, null,
  ...
  private NodeData initJcrSystemNode(NodeData root) throws RepositoryException {
    ...
    TransientNodeData sysNodeData = TransientNodeData.createNodeData(root,
        Constants.JCR_SYSTEM, Constants.NT_UNSTRUCTURED, Constants.SYSTEM_UUID);
    
    TransientPropertyData primaryType = TransientPropertyData.createPropertyData(
        sysNodeData, Constants.JCR_PRIMARYTYPE, PropertyType.NAME, false);
    primaryType.setValue(new TransientValueData(sysNodeData.getPrimaryTypeName()));
    
    changesLog.add(ItemState.createAddedState(sysNodeData))
    ...
    if (addACL) {
      ...
      // jcr:mixinTypes
      TransientPropertyData exoMixinTypes = TransientPropertyData
          .createPropertyData(sysNodeData, Constants.JCR_MIXINTYPES, PropertyType.NAME, false);
      ...
      TransientPropertyData exoOwner = TransientPropertyData.createPropertyData(
          sysNodeData, Constants.EXO_OWNER, PropertyType.STRING, false, 
          new TransientValueData(acl.getOwner()));
      TransientPropertyData exoPerms = TransientPropertyData.createPropertyData(
          sysNodeData, Constants.EXO_PERMISSIONS, ExtendedPropertyType.PERMISSION, true);
    ...
    TransientNodeData versionStorageNodeData = TransientNodeData
        .createNodeData(sysNodeData, Constants.JCR_VERSIONSTORAGE,
            Constants.EXO_VERSIONSTORAGE, Constants.VERSIONSTORAGE_UUID);

    TransientPropertyData vsPrimaryType = TransientPropertyData
        .createPropertyData(versionStorageNodeData, Constants.JCR_PRIMARYTYPE,
            PropertyType.NAME, false);
    vsPrimaryType.setValue(new TransientValueData(versionStorageNodeData
        .getPrimaryTypeName()));
    ...
    // TODO To catch exceptions caused by already existing nodes (jcr:system, jcr:versionStorage)
    return sysNodeData;
  }
}
