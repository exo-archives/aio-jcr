...
public class XPathQueryBuilder implements XPathVisitor, XPathTreeConstants {
   ...
   */
  static final InternalQName     JCR_XMLTEXT      = new InternalQName(Constants.NS_JCR_URI,
                                                                      "xmltext");
   ...
   */
  static final InternalQName     XS_DATETIME      = new InternalQName("http://www.w3.org/2001/XMLSchema",
                                                                      "dateTime");
   ...
   */
  static final InternalQName     JCR_CONTAINS     = new InternalQName(Constants.NS_JCR_URI,
                                                                      "contains");
   ...
   */
  static final InternalQName     REP_SIMILAR      = new InternalQName(Constants.NS_REP_URI,
                                                                      "similar");

  /**
   * Name for exo:spellcheck
   */
  static final InternalQName     REP_SPELLCHECK   = new InternalQName(Constants.NS_REP_URI,
                                                                      "spellcheck");
...
/**
   ...
   */
  private static final Map       parsers          = new ReferenceMap(ReferenceMap.WEAK,
                                                                     ReferenceMap.WEAK);
    ...
    case JJTSTEPEXPR:
      if (isAttributeAxis(node)) {
        if (queryNode.getType() == QueryNode.TYPE_RELATION
          ...
          // is null expression
          RelationQueryNode isNull = factory.createRelationQueryNode(queryNode,
        ...
        if (queryNode.getType() == QueryNode.TYPE_PATH) {
          ...
          RelationQueryNode tmp = factory.createRelationQueryNode(null,
                                                                  RelationQueryNode.OPERATION_NOT_NULL);
    ...
    if (node.jjtGetNumChildren() > 0) {
      ...
      if (child.getId() == JJTQNAME || child.getId() == JJTQNAMEFORITEMTYPE) {
        ...
        if (queryNode.getType() == QueryNode.TYPE_LOCATION) {
          ...
          ((RelationQueryNode) queryNode).addPathElement(new QPathEntry(RelationQueryNode.STAR_NAME_TEST,
                                                                        0));
        } else if (queryNode.getType() == QueryNode.TYPE_TEXTSEARCH) {
          ((TextsearchQueryNode) queryNode).addPathElement(new QPathEntry(RelationQueryNode.STAR_NAME_TEST,
                                                                          0));
        }
      } else {
  ...
  private void createExpression(SimpleNode node, NAryQueryNode queryNode) {
    ...
    rqn.acceptOperands(new DefaultQueryNodeVisitor() {
      public Object visit(PropertyFunctionQueryNode node, Object data) {
        ...
        if ((functionName.equals(PropertyFunctionQueryNode.LOWER_CASE) || functionName.equals(PropertyFunctionQueryNode.UPPER_CASE))
      ...
      if (FN_NOT.equals(funName) || FN_NOT_10.equals(funName)) {
        ...
        if (node.jjtGetNumChildren() == 3) {
          if (queryNode instanceof NAryQueryNode) {
            ...
            if (literal.getId() == JJTSTRINGLITERAL) {
              TextsearchQueryNode contains = factory.createTextsearchQueryNode(queryNode,
        ...
        if (node.jjtGetNumChildren() == 3) {
          if (queryNode instanceof NAryQueryNode) {
            RelationQueryNode like = factory.createRelationQueryNode(queryNode,
                                                                     RelationQueryNode.OPERATION_LIKE);
        ...
        if (node.jjtGetNumChildren() == 3) {
          if (queryNode instanceof NAryQueryNode) {
            NAryQueryNode parent = (NAryQueryNode) queryNode;
            RelationQueryNode rel = factory.createRelationQueryNode(parent,
                                                                    RelationQueryNode.OPERATION_SIMILAR);
            ...
            if (rel.getStringValue() == null) {
              exceptions.add(new InvalidQueryException("Second argument for exo:similar() must be of type string"));
            }
          } else {
