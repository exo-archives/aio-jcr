...
class QueryFormat implements QueryNodeVisitor, QueryConstants {
  ...
  public Object visit(QueryRootNode node, Object data) {
    ...
    try {
      ...
      } else if (steps.length == 1 && steps[0].getIncludeDescendants()
  ...
  public Object visit(OrQueryNode node, Object data) {
    ...
    if (node.getParent() instanceof LocationStepQueryNode
        || node.getParent() instanceof AndQueryNode || node.getParent() instanceof NotQueryNode) {
      bracket = true;
    }
    if (bracket) {
      sb.append("(");
    ...
    try {
      ...
      if (node.getOperation() == OPERATION_EQ_VALUE || node.getOperation() == OPERATION_EQ_GENERAL) {
        ...
        appendValue(node, sb);
          ...
          || node.getOperation() == OPERATION_GE_GENERAL) {
        sb.append(propName);
        sb.append(" >= ");
        appendValue(node, sb);
      } else if (node.getOperation() == OPERATION_GT_VALUE
          || node.getOperation() == OPERATION_GT_GENERAL) {
        sb.append(propName);
        sb.append(" > ");
        appendValue(node, sb);
      } else if (node.getOperation() == OPERATION_LE_VALUE
          || node.getOperation() == OPERATION_LE_GENERAL) {
        sb.append(propName);
        sb.append(" <= ");
        appendValue(node, sb);
        ...
        appendValue(node, sb);
          ...
          || node.getOperation() == OPERATION_LT_GENERAL) {
        sb.append(propName);
        sb.append(" < ");
        appendValue(node, sb);
      } else if (node.getOperation() == OPERATION_NE_VALUE
          || node.getOperation() == OPERATION_NE_GENERAL) {
        sb.append(propName);
        sb.append(" <> ");
        appendValue(node, sb);
   ...
   */
  private static void appendName(InternalQName name, LocationFactory resolver, StringBuffer b) throws NamespaceException {
