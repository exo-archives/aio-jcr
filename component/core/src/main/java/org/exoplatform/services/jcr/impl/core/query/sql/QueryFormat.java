...
//    private final NamespaceResolver resolver;
  private LocationFactory locationFactory;
        ...
        try {
            ...
            if (selectProps.length == 0) {
                ...
                for (int i = 0; i < selectProps.length; i++) {
                    ...
                    appendName(selectProps[i], locationFactory, sb);
                    comma = ",";
                }
            }
            ...
            for (Iterator it = nodeTypes.iterator(); it.hasNext(); ntCount++) {
                ...
                sb.append(comma).append(" ");
                appendName(nt, locationFactory, sb);
                comma = ",";
            }

            if (ntCount == 0) {
                sb.append(" ");
                sb.append(locationFactory.createJCRName(Constants.NT_BASE).getAsString());
            }
    ...
    public Object visit(ExactQueryNode node, Object data) {
        StringBuffer sb = (StringBuffer) data;
        try {
            appendName(node.getPropertyName(), locationFactory, sb);
        } catch (RepositoryException e) {
            exceptions.add(e);
        }
    ...
    public Object visit(RelationQueryNode node, Object data) {
        ...
        try {
            if (node.getOperation() == OPERATION_EQ_VALUE || node.getOperation() == OPERATION_EQ_GENERAL) {
                appendName(node.getProperty(), locationFactory, sb);
                sb.append(" = ");
                appendValue(node, sb);
        ...
        if (specs.length > 0) {
            try {
                ...
                for (int i = 0; i < specs.length; i++) {
                    sb.append(comma).append(" ");
                    appendName(specs[i].getProperty(), locationFactory, sb);
    ...
    private static void appendName(InternalQName name,
        ...
        b.append(locationFactory.createJCRName(name).getAsString());
        if (quote) {
            b.append('"');
        }
    }
