      ...
      if (base.isMixin()) {
        ...
        Term t = new Term(FieldNames.PROPERTIES,
                          FieldNames.createNamedValue(mixinTypesField,
                                                      resolver.createJCRName(node.getValue())
                                                              .getAsString()));
        terms.add(t);
      } else {
      ...
      for (NodeTypeData nodeTypeData : allTypes) {
        ...
        if (Arrays.asList(superTypes).contains(base.getName())) {
          ...
          if (nodeTypeData.isMixin()) {
            ...
            t = new Term(FieldNames.PROPERTIES, FieldNames.createNamedValue(primaryTypeField,
                                                                            ntName));
          }
          terms.add(t);
        }
  ...
  public Object visit(TextsearchQueryNode node, Object data) {
    try {
      ...
      if (relPath == null || !node.getReferencesProperty()) {
        ...
        fieldname = FieldNames.createFullTextFieldName(resolver.createJCRName(propName)
                                                               .getAsString());
      }
    ...
    if (node.getIncludeDescendants()) {
      if (nameTest != null) {
        ...
        if (predicates.length > 0) {
          ...
          Query subQuery = null;
          try {
            subQuery = createMatchAllQuery(resolver.createJCRName(Constants.JCR_PRIMARYTYPE)
                                                   .getAsString());
          } catch (RepositoryException e) {
            // will never happen, prefixes are created when unknown
          }
      ...
      if (nameTest != null) {
        andQuery.add(new ChildAxisQuery(sharedItemMgr,
                                        context,
                                        nameTest.getTerm().text(),
                                        node.getIndex()), Occur.MUST);
      } else {
        // select child nodes
