...
public final class SessionChangesLog extends PlainChangesLogImpl {
  ...
  /**
   * Used to store count of child nodes.
   */
    ...
    for (int i = items.size() - 1; i >= 0; i--) {
      ...
      if (qPath.isDescendantOf(rootPath) || item.getAncestorToSave().isDescendantOf(rootPath)
        ...
        if (item.isNode() && item.isPersisted()) {
          Integer childCount = childNodesCount.get(item.getData().getParentIdentifier());
          if (childCount != null) {
            if (item.isDeleted())
              ++childCount;
        ...
        if (item.getData().isNode()) {
          Map<String, ItemState> children = lastChildNodeStates.get(item.getData()
                                                                        .getParentIdentifier());
          if (children != null)
            children.remove(item.getData().getIdentifier());
        } else {
          Map<String, ItemState> children = lastChildPropertyStates.get(item.getData()
                                                                            .getParentIdentifier());
          if (children != null)
            children.remove(item.getData().getIdentifier());
        }
  ...
  public int getChildNodesCount(String rootIdentifier) {
    Integer childCount = childNodesCount.get(rootIdentifier);
  ...
  public Collection<ItemState> getLastChildrenStates(ItemData rootData, boolean forNodes) {
    Map<String, ItemState> children = forNodes
        ? lastChildNodeStates.get(rootData.getIdentifier())
        : lastChildPropertyStates.get(rootData.getIdentifier());
   ...
   * @throws IllegalPathException
   */
  public ItemState findItemState(QPath rootPath,
                                 Boolean isPersisted,
                                 Boolean orAncestor,
                                 int... states) throws IllegalPathException {
    List<ItemState> allStates = getAllStates();
    ...
    for (int i = allStates.size() - 1; i >= 0; i--) {
      ...
      if (byState
          ...
          && ((orAncestor != null && orAncestor ? rootPath.isDescendantOf(istate.getData()
                                                                                .getQPath()) : true) || rootPath.equals(istate.getData()
                                                                                                                              .getQPath()))) {
        return istate;
      }
    }
    ...
    public boolean equals(Object obj) {
      ...
      if (name == null) {
        if (other.name != null)
          ...
          || !name.getNamespace().equals(other.name.getNamespace())
          || name.getIndex() != other.name.getIndex())
        return false;
