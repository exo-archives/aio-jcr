 ...
 * delete operations on the index.
 * <p/>
 * The persistent indexes are merged from time to time. The merge behaviour is configurable using
 * the methods: {@link SearchIndex#setMaxMergeDocs(int)}, {@link SearchIndex#setMergeFactor(int)}
 * and {@link SearchIndex#setMinMergeDocs(int)}. For detailed description of the configuration
 * parameters see also the lucene <code>IndexWriter</code> class.
 * <p/>
 * This class is thread-safe.
 * <p/>
 * Note on implementation: Multiple modifying threads are synchronized on a <code>MultiIndex</code>
 * instance itself. Sychronization between a modifying thread and reader threads is done using
 * {@link #updateMonitor} and {@link #updateInProgress}.
 */
public class MultiIndex {
   ...
   *          the search handler
   * @param excludedIDs
   *          Set&lt;NodeId> that contains uuids that should not be indexed nor further traversed.
   * @param mapping
   *          the namespace mapping to use
   * @throws IOException
   *           if an error occurs
   */
   ...
   * 
   * @param remove
   *          Iterator of <code>UUID</code>s that identify documents to remove
   * @param add
   *          Iterator of <code>Document</code>s to add. Calls to <code>next()</code> on this
   ...
   * the indexes in <code>indexNames</code>.
   * <p/>
   * Note: the number of <code>IndexReaders</code> returned by this method is not necessarily the
   * same as the number of index names passed. An index might have been deleted and is not reachable
   * anymore.
   * 
   * @param indexNames
   ...
   * 
   * @param indexName
   *          the name of the index to open, or <code>null</code> if an index with a new name should
   *          be created.
   ...
   * @param index
   *          the new index that is the result of a merge of the indexes to replace.
   * @param deleted
   *          <code>Term</code>s that identify documents that must be deleted in <code>index</code>.
   * @throws IOException
   ...
   * @return the index document.
   * @throws RepositoryException
   *           if an error occurs while reading from the workspace or if there is no node with
   *           <code>id</code>.
   */
   ...
   * deleted right away, but postponed to the transaction commit.
   * <p/>
   * This method does not close the index, but rather expects that the index has already been
   * closed.
   * 
   * @param index
   *          the index to delete.
   ...
   * @throws IOException
   *           if an error occurs while executing the action or appending the action to the redo
   *           log.
   */
  ...
  public abstract static class Action {
     ...
     * @throws IOException
     *           if the action fails due to some I/O error in the index or some other error.
     */
  ...
  private static class AddIndex extends Action {
     ...
     * @param indexName
     *          the name of the index to add, or <code>null</code> if an index with a new name
     *          should be created.
     */
