 ...
 * class. <p/> This class is thread-safe. <p/> Note on implementation: Multiple
...
class MultiIndex {
   ...
   */
  private static Log                     log                  = ExoLogger.getLogger("jcr.MultiIndex");
   ...
   * that reads from all indexes, including volatile and persistent indexes.
   */
   ...
   */
  MultiIndex(File indexDir, SearchIndex handler, WorkspacePersistentDataManager dataManager, String rootIdentifier)
      throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < indexNames.size(); i++) {
        ...
        PersistentIndex index = new PersistentIndex(indexNames.getName(i), sub, false, handler.getAnalyzer(), cache);
        ...
        index.setMaxFieldLength(handler.getMaxFieldLength()); // by 410647
                                                              // revision
   ...
   *          <code>null</code>, to indicate that a node could not be indexed
   *          successfully.
   */
  synchronized void update(final Iterator remove, final Iterator add) throws IOException {
   ...
   * <code>indexNames</code>. An <code>IndexListener</code> is registered
   * and notified when documents are deleted from one of the indexes in
   * <code>indexNames</code>. <p/> Note: the number of
   * <code>IndexReaders</code> returned by this method is not necessarily the
   * same as the number of index names passed. An index might have been deleted
   ...
   */
  synchronized PersistentIndex getOrCreateIndex(String indexName, boolean create) throws IOException {
    // check existing
    for (Iterator it = indexes.iterator(); it.hasNext();) {
    ...
    PersistentIndex index = new PersistentIndex(indexName, sub, true, handler.getAnalyzer(), cache);
    index.setMaxMergeDocs(handler.getMaxMergeDocs());
    index.setMergeFactor(handler.getMergeFactor());
    index.setMinMergeDocs(handler.getMinMergeDocs());
    index.setMaxFieldLength(handler.getMaxFieldLength()); // 410647
    index.setUseCompoundFile(handler.getUseCompoundFile());

    // add to list of open indexes and return it
    indexes.add(index); // 410647
    return index;
  }
   ...
   */
  synchronized void replaceIndexes(String[] obsoleteIndexes, PersistentIndex index, Collection deleted)
      throws IOException {
    synchronized (updateMonitor) {
      updateInProgress = true;
    }
    try {
      ...
      /*
       * // add new indexes.add(index); indexNames.addName(index.getName());
       * merger.indexAdded(index.getName(), index.getNumDocuments());
  ...
  IndexReader getIndexReader() throws IOException {
    synchronized (updateMonitor) {
      ...
      if (multiReader == null) {
        ...
        ReadOnlyIndexReader[] readers = (ReadOnlyIndexReader[]) readerList.toArray(new ReadOnlyIndexReader[readerList
            .size()]);
  ...
  /*
   * only JR implementation Returns the namespace mappings of this search index.
   * @return the namespace mappings of this search index. NamespaceMappings
   * getNamespaceMappings() { return nsMappings; }
   */
  ...
  Document createDocument(NodeData node) throws RepositoryException {
    return handler.createDocument(node);
  }
   ...
   * because (windows) file handles are still open, the directory is marked for
   * future deletion. <p/> This method does not close the index, but rather
   * expects that the index has already been closed.
   * 
   ...
   * @throws IOException if an error occurs while updating the index. private
   *           int internalRemoveDocument(Term idTerm) throws IOException { //
   *           if the document cannot be deleted from the volatile index //
   *           delete it from one of the persistent indexes. int num =
   *           volatileIndex.removeDocument(idTerm); if (num == 0) { for (int i =
   *           indexes.size() - 1; i >= 0; i--) { PersistentIndex index =
   *           (PersistentIndex) indexes.get(i); num =
   *           index.removeDocument(idTerm); if (num > 0) { return num; } } }
   *           else { return num; } return 0; } /** Unsynchronized
   *           implementation to add a document to the index.
   * @param doc the document to add.
   * @throws IOException if an error occurs while adding the document to the
   *           index. private void internalAddDocument(Document doc) throws
   *           IOException { volatileIndex.addDocument(doc); if
   *           (volatileIndex.getRedoLog().getSize() >=
   *           handler.getMinMergeDocs()) { long time =
   *           System.currentTimeMillis(); commit(); time =
   *           System.currentTimeMillis() - time; log.debug("Committed in-memory
   *           index in " + time + "ms."); } } /** Commits the volatile index to
   *           a persistent index, commits persistent indexes (persist
   *           deletions) and finally merges indexes if necessary.
   * @throws IOException if an error occurs. private void commit() throws
   *           IOException { // check if volatile index contains documents at
   *           all if (volatileIndex.getIndexReader().numDocs() > 0) { File sub =
   *           newIndexFolder(); String name = sub.getName(); PersistentIndex
   *           index = new PersistentIndex(name, sub, true, handler
   *           .getAnalyzer(), cache);
   *           index.setMaxMergeDocs(handler.getMaxMergeDocs());
   *           index.setMergeFactor(handler.getMergeFactor());
   *           index.setMinMergeDocs(handler.getMinMergeDocs());
   *           index.setUseCompoundFile(handler.getUseCompoundFile());
   *           index.copyIndex(volatileIndex); // if merge has been successful
   *           add index indexes.add(index); indexNames.addName(name);
   *           indexNames.write(indexDir); merger.indexAdded(index.getName(),
   *           index.getNumDocuments()); // check if obsolete indexes can be
   *           deleted // todo move to other place? attemptDelete(); } // commit
   *           persistent indexes for (int i = indexes.size() - 1; i >= 0; i--) {
   *           PersistentIndex index = (PersistentIndex) indexes.get(i);
   *           index.commit(); // check if index still contains documents if
   *           (index.getNumDocuments() == 0) { indexes.remove(i);
   *           indexNames.removeName(index.getName());
   *           indexNames.write(indexDir); index.close(); deleteIndex(index); } } //
   *           reset redo log volatileIndex.getRedoLog().clear(); // create new
   *           volatile index volatileIndex = new
   *           VolatileIndex(handler.getAnalyzer(), volatileIndex
   *           .getRedoLog());
   *           volatileIndex.setUseCompoundFile(handler.getUseCompoundFile());
   *           volatileIndex.setBufferSize(handler.getBufferSize()); }
   */
   ...
   */
  private void createIndex(NodeData node) throws IOException, RepositoryException {
  ...
  private static class AddIndex extends Action {
     ...
     * @param indexName the name of the index to add, or <code>null</code> if
  ...
  private static class AddNode extends Action {
    ...
    private static final int ENTRY_LENGTH = Long.toString(Long.MAX_VALUE).length() + Action.ADD_NODE.length()
                                              + IdGenerator.IDENTIFIER_LENGTH + 2;
  ...
  private static class CreateIndex extends Action {
    ...
    /**
     * Indicates if the index is forced to be created. That is, existing index
     * data is deleted.
     */
     ...
     * @param indexName the name of the index to add, or <code>null</code> if
     *          an index with a new name should be created.
     * @param create if <code>true</code> existing index data is overwritten.
     */
