...
class MultiIndex {
      ...
      WorkspacePersistentDataManager dataManager, String rootUUID) throws IOException
  {
   ...
   try
   {
    ...
    for (int i = 0; i < indexNames.size(); i++)
    {
     ...
     PersistentIndex index = new PersistentIndex(indexNames.getName(i), sub,
            false, handler.getAnalyzer(), cache);
     ...
     index.setMaxFieldLength(handler.getMaxFieldLength()); //by 410647 revision
  ...
  synchronized PersistentIndex getOrCreateIndex(String indexName, boolean create)
    ...
    PersistentIndex index = new PersistentIndex(indexName, sub, true, handler
        .getAnalyzer(), cache);
    index.setMaxMergeDocs(handler.getMaxMergeDocs());
    index.setMergeFactor(handler.getMergeFactor());
    index.setMinMergeDocs(handler.getMinMergeDocs());
  ...
  synchronized void replaceIndexes(String[] obsoleteIndexes,
      PersistentIndex index, Collection deleted) throws IOException {
    synchronized (updateMonitor) {
            updateInProgress = true;
        }
        try {
  ...
  */
  ...
  private void commit() throws IOException {
    ...
    if (volatileIndex.getIndexReader().numDocs() > 0) {
      ...
      PersistentIndex index = new PersistentIndex(name, sub, true, handler
          .getAnalyzer(), cache);
      index.setMaxMergeDocs(handler.getMaxMergeDocs());
      index.setMergeFactor(handler.getMergeFactor());
      index.setMinMergeDocs(handler.getMinMergeDocs());
    ...
    volatileIndex = new VolatileIndex(handler.getAnalyzer(), volatileIndex
        .getRedoLog());
    volatileIndex.setUseCompoundFile(handler.getUseCompoundFile());
    volatileIndex.setBufferSize(handler.getBufferSize());
