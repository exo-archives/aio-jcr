  ...
  protected String handleAddException(Exception e, ItemData item) throws RepositoryException,
    ...
    if (errMessage != null) {
      ...
      if (umsg.indexOf(conn.JCR_FK_ITEM_PARENT) >= 0) {
        ...
        message += "Property is not exist but the value is being created. Condition: property ID. "
            + itemInfo;
        ...
        message += "[FATAL] Value already exists with the ValueID. Impossible state, check is ValueID is autoincremented. "
            + itemInfo;
        throw new RepositoryException(message, e);
      } else if (umsg.indexOf(conn.JCR_PK_REF) >= 0) {
        message += "Reference chain already exists. Condition: node ID, property ID, order number. "
            + itemInfo;
        throw new RepositoryException(message, e);
      } else if (umsg.indexOf(conn.JCR_IDX_REF_PROPERTY) >= 0) {
        message += "Referenceable property value already exists. Condition: property ID, order number. "
            + itemInfo;
        throw new RepositoryException(message, e);
      }
    }
  ...
  protected String handleDeleteException(Exception e, ItemData item) throws RepositoryException,
    ...
    if (errMessage != null) {
      ...
      if (umsg.indexOf(conn.JCR_FK_ITEM_PARENT) >= 0) {
        ...
        message += "[FATAL] Can not delete property item till it contains values. Condition: property ID. "
            + itemInfo;
        throw new RepositoryException(message, e);
      }
    }
  ...
  protected String handleUpdateException(Exception e, ItemData item) throws RepositoryException,
    ...
    if (errMessage != null)
      ...
      if (errMessage.toLowerCase().toUpperCase().indexOf(conn.JCR_FK_VALUE_PROPERTY) >= 0) {
        message += "Property is not exists but the value is being created. Condition: property ID. "
            + itemInfo;
        throw new RepositoryException(message, e);
