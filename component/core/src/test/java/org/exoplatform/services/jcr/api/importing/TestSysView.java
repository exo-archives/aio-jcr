 ...
 */
public class TestSysView extends JcrAPIBaseTest {
  ...
  public void testExportUuid_IMPORT_UUID_CREATE_NEW() throws Exception {
    ...
    sysview.getSession().importXML(importTarget.getPath(), new FileInputStream(xmlContent), ImportUUIDBehavior.IMPORT_UUID_CREATE_NEW);
    ...
    assertFalse("Uuids must be different. " + importedSNSUuid + " != " + importedUuid, importedSNSUuid.equals(importedUuid));
    
    // ...temp check
    InputStream anyBinary = importTarget.getNode(SOURCE_NAME + "[2]/jcr:content").getProperty("anyBinary").getStream();
    assertEquals("Stream length must be same", BIN_STRING.length(), anyBinary.available());
    assertEquals("Stream content must be same", BIN_STRING, 
        importTarget.getNode(SOURCE_NAME + "[2]/jcr:content").getProperty("anyBinary").getString());
  }
  ...
  public void testExportUuid_IMPORT_UUID_COLLISION_REMOVE_EXISTING() throws Exception {
    ...
    sysview.getSession().importXML(importTarget.getPath(), new FileInputStream(xmlContent), ImportUUIDBehavior.IMPORT_UUID_COLLISION_REMOVE_EXISTING);
    ...
    // try one more (for same-name sibling nodes test), mus replace before imported node
    sysview.getSession().importXML(importTarget.getPath(), new FileInputStream(xmlContent), ImportUUIDBehavior.IMPORT_UUID_COLLISION_REMOVE_EXISTING);
  ...
  public void testExportUuid_IMPORT_UUID_COLLISION_REPLACE_EXISTING() throws Exception {
    ...
    sysview.getSession().importXML(importTarget.getPath(), new FileInputStream(xmlContent), ImportUUIDBehavior.IMPORT_UUID_COLLISION_REPLACE_EXISTING);
    ...
    assertFalse("A imported node must has no property 'New property 1, boolean' " + target.getPath(), 
        target.hasProperty("jcr:content/New property 1, boolean"));
    ...
    assertTrue("Uuid of SNS replaced node must be different. " + importedSNSUuid + " != " + importedSNSUuid, importedSNSUuid.equals(importedSNSUuid));
  ...
  public void testExportUuid_IMPORT_UUID_COLLISION_THROW() throws Exception {
    ...
    try {
      sysview.getSession().importXML(importTarget.getPath(), new FileInputStream(xmlContent), ImportUUIDBehavior.IMPORT_UUID_COLLISION_THROW);
