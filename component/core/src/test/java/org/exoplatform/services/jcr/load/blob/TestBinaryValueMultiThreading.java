...
public class TestBinaryValueMultiThreading extends JcrAPIBaseTest {
  ...
  protected class TestJCRClient extends Thread {
    ...
    public void run() {
      ...
      try {
        clientSession = (SessionImpl) repository.login(new CredentialsImpl("exo",
                                                                           "exo".toCharArray()),
                                                       "ws");
        ...
        for (int i = 0; i < FILES_COUNT; i++) {
          ...
          compareStream(new FileInputStream(LOCAL_SMALL_FILE),
                        testLocalSmallFiles.getProperty("smallFile" + i + "/jcr:content/jcr:data")
                                           .getStream());
          compareStream(new FileInputStream(LOCAL_BIG_FILE),
                        testLocalBigFiles.getProperty("bigFile" + i + "/jcr:content/jcr:data")
                                         .getStream());
