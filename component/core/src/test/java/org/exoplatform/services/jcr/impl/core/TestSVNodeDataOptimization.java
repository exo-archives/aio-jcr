...
public class TestSVNodeDataOptimization extends JcrImplBaseTest {
  ...
  public void testSVNodeData() throws Exception {
    ...
    WorkspaceEntry ws1_restore = makeWorkspaceEntry("ws1_restored",
                                                    isMultiDB(session) ? "jdbcjcr2export3"
                                                                      : "jdbcjcr",
                                                    backup,
                                                    ses);
  ...
  private WorkspaceEntry makeWorkspaceEntry(String name, String sourceName, File sysViewFile, SessionImpl ses) {
    WorkspaceEntry ws1e = (WorkspaceEntry) ses.getContainer()
                                                  .getComponentInstanceOfType(WorkspaceEntry.class);

    WorkspaceEntry ws1back = new WorkspaceEntry();
    ws1back.setName(name);
    ws1back.setUniqueName(((RepositoryImpl) ses.getRepository()).getName() + "_"
        + ws1back.getName());
    ...
    wieParams.add(new SimpleParameterEntry(RestoreWorkspaceInitializer.RESTORE_PATH_PARAMETER,
                                           sysViewFile.getPath()));
  ...
  private boolean isMultiDB(SessionImpl session) {
    ...
    throw new RuntimeException("Can not get property 'multi-db' in configuration on workspace '"
        + ws1e.getName() + "'");
  }
