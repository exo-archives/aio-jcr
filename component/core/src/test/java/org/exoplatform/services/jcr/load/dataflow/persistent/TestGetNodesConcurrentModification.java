...
public class TestGetNodesConcurrentModification extends JcrImplBaseTest {
              ...
              while (progress && librIter.hasNext()) {
                ...
                while (progress && filesIter.hasNext()) {
                  ...
                  try {
                    ...
                    String msg = getName()
                        + " >>> library jcr:content/fileData/jcr:data read error " + e
                        + ". Last node " + nodePath;
                    log.error(msg, e);
                    addFail(msg);
                  }
  ...
  public void setUp() throws Exception {
    ...
    contentFile = new URL("file:///"
        + new File("src/test/resources/index/test_index.doc").getAbsolutePath());
    initDB();
  }
  ...
  private void initDB() throws Exception {
    if (!session.getRootNode().hasNode("concurrent_node")) {
      ...
      for (int l = 1; l <= 4; l++) {
        ...
        log.info(subChild.getPath() + " childs added "
            + (System.currentTimeMillis() - startLibrary) + "ms");
        startLibrary = System.currentTimeMillis();
        log.info(subChild.getPath() + " childs will be saved, wait few minutes...");
        session.save();
        log.info(subChild.getPath() + " childs saved "
            + (System.currentTimeMillis() - startLibrary) + "ms");
      ...
      session.save();
      log.info("Items created " + itemsCount + ", " + (System.currentTimeMillis() - startTime)
          + "ms");
    }
  }
