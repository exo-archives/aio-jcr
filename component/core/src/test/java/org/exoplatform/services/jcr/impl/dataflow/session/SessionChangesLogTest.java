...
public class SessionChangesLogTest extends JcrImplBaseTest {
    ...
    Thread thread = new Thread() {
      ...
      public void run() {
        ...
        try {
          ...
          chlog[0] = new SessionChangesLogInfo(new SessionChangesLog(userSession.getId()),
                                               userSession.getId());
  ...
  private SessionImpl getRegisteredSession(String sessionId) {
    ...
    SessionRegistry sreg = (SessionRegistry) session.getContainer()
  ...
  public void testSameSession() {
    ...
    assertEquals("Session must be same as given id owns",
                 session,
  ...
  public void testAnotherThreadSession() throws Exception {
    ...
    assertEquals("Session must be same as given id owns",
                 chlog.getSessionId(),
                 getRegisteredSession(chlog.getLog().getSessionId()).getId());
  }
  ...
  public void testSessionLogout() throws Exception {
    ...
    assertNull("No session should be linked to the log", getRegisteredSession(chlog.getLog()
                                                                                   .getSessionId()));
  }
  ...
  public void testMultipleSessionsLogout() throws Exception {
    ...
    for (int i = 1; i <= 1000; i++) {
      ...
      TransientNodeData ndata = TransientNodeData.createNodeData(parent,
                                                                 qname,
                                                                 Constants.NT_UNSTRUCTURED);

      // jcr:primaryType
      TransientPropertyData ndpt = TransientPropertyData.createPropertyData(ndata,
