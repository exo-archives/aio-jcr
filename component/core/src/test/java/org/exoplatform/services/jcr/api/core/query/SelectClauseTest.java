...
public class SelectClauseTest extends AbstractQueryTest {
  ...
  public void testSelectSQL() throws RepositoryException {
    ...
    String sql = "SELECT myvalue FROM " + ntBase + " WHERE " + "jcr:path LIKE '" + testRoot
        + "/%' AND myvalue IS NOT NULL";
    Query q = superuser.getWorkspace().getQueryManager().createQuery(sql, Query.SQL);
    QueryResult result = q.execute();
    checkResult(result, 2);

    sql = "SELECT myvalue FROM " + ntBase + " WHERE jcr:path LIKE '" + testRoot + "/%'"
  ...
  public void testPropertyCountSQL() throws RepositoryException {
    ...
    sql = "SELECT * FROM " + ntBase + " WHERE jcr:path LIKE '" + testRoot + "/%'"
        + " AND myvalue LIKE '%'";
    q = superuser.getWorkspace().getQueryManager().createQuery(sql, Query.SQL);
    result = q.execute();
    checkResult(result, 2, 2);
