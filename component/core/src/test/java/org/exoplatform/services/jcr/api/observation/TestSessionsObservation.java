...
public class TestSessionsObservation extends JcrAPIBaseTest {
  ...
  public void testSessionOpen() throws RepositoryException {
    ...
    try {
      sessionWs1.getWorkspace().getObservationManager().addEventListener(listener,
                                                                         Event.NODE_ADDED,
                                                                         testRootWs1.getPath()
                                                                             + "/n1",
                                                                         false,
  ...
  public void testMultipleSessionClosed() throws Exception {
    ...
    SimpleListener addListener = new SimpleListener("testMultipleSessionClosed__add_nt:file",
                                                    log,
                                                    0);
    SimpleListener removeListener = new SimpleListener("testMultipleSessionClosed__remove", log, 0);

    try {
      sessionWs1.getWorkspace()
      ...
      for (int i = 0; i < sessionCount; i++) {
        ...
        try {
          ...
          Node n1 = anotherWs1.getRootNode().getNode(testRootWs1.getName()).addNode("n" + i,
                                                                                    "nt:file"); // /
          // observationTest
          // /
          // n1
  ...
  public void testListenerRemoved() throws Exception {
    ...
    try {
      sessionWs1.getWorkspace().getObservationManager().addEventListener(listener1,
                                                                         Event.NODE_ADDED,
                                                                         testRootWs1.getPath()
                                                                             + "/n1",
                                                                         true,
      ...
      sessionWs1.getWorkspace().getObservationManager().addEventListener(listener2,
