...
public class SingleDbJDBCConnectionFindTest extends JDBCConnectionBaseTest{
	...
	public void testFindChildNodesByParentIdentifier() throws Exception{
		...
		rsRemote.next();
		makeFindDB("select * from JCR_SITEM"
						+ " where I_CLASS=1 and CONTAINER_NAME='myContainer' and PARENT_ID='1235'" + " order by N_ORDER_NUM");
	...
	public void testFindChildPropertiesByParentIdentifier () throws Exception{
		...
		rsRemote.next();
		makeFindDB("select * from JCR_SITEM"
						+ " where I_CLASS=2 and CONTAINER_NAME='mycontainer' and PARENT_ID='123456'" + " order by ID");
		assertEquals(rs.getString("PARENT_ID"),rsRemote.getString("PARENT_ID"));	
	}
