...
public class TestAggregateRules extends BaseQueryTest {
  ...
  public void testAdditionFile() throws Exception{
    ...
    IndexingConfigurationImpl indexingConfigurationImpl = (IndexingConfigurationImpl) defaultSearchIndex.getIndexingConfig();
    ...
    content.setProperty("jcr:data", new ByteArrayInputStream("The quick brown fox jump over the lazy dog".getBytes()));
    session.save();
    
    QueryManager qman =this.workspace.getQueryManager();
    
    Query q = qman.createQuery("SELECT * FROM nt:file "
        + " WHERE  CONTAINS(., 'fox')", Query.SQL); 
    
    QueryResult res = q.execute();
    
    assertEquals(1,res.getNodes().getSize());
    
    q = qman.createQuery("SELECT * FROM nt:resource "
        + " WHERE  CONTAINS(., 'fox')", Query.SQL); 
