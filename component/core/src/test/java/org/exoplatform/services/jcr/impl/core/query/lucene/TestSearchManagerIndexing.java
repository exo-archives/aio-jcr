...
public class TestSearchManagerIndexing extends JcrImplBaseTest {
  ...
  public void testAdditionNode() throws Exception {
    ...
    TermQuery prop1 = new TermQuery(new Term(FieldNames.PROPERTIES, getIndexPrefix(si, "") + "prop"
        + '\uFFFF' + "string value"));
    TermQuery prop2 = new TermQuery(new Term(FieldNames.PROPERTIES, getIndexPrefix(si, "jcr")
        + "primaryType" + '\uFFFF' + getIndexPrefix(si, "nt") + "unstructured"));
    ...
    TermQuery fullprop1 = new TermQuery(new Term(FieldNames.createFullTextFieldName(getIndexPrefix(si,
                                                                                                   "")
                                                     + "prop"),
                                                 "string"));
    TermQuery fullprop2 = new TermQuery(new Term(FieldNames.createFullTextFieldName(getIndexPrefix(si,
                                                                                                   "")
                                                     + "prop"),
                                                 "value"));
  ...
  public void testAddMixinType() throws Exception {
    ...
    TermQuery prop1 = new TermQuery(new Term(FieldNames.PROPERTIES, getIndexPrefix(si, "jcr")
        + "primaryType" + '\uFFFF' + getIndexPrefix(si, "nt") + "unstructured"));
    BooleanQuery compl = new BooleanQuery();
    compl.add(name, Occur.MUST);
    compl.add(prop1, Occur.MUST);
  ...
  public void setUp() throws Exception {
    super.setUp();
    manager = (SearchManager) this.session.getContainer()
                                          .getComponentInstanceOfType(SearchManager.class);
  }

  private String getIndexPrefix(SearchIndex si, String stPref) throws RepositoryException,
                                                              NamespaceException {
