...
public class AddSameNameNodeTest extends BaseUsecasesTest {
  ...
  public void testAddSameNameNode() throws Exception {
    ...
    try {
      assertEquals("Content must be equals", testRoot.getProperty("file1/jcr:content/any property")
                                                     .getString(), "any content");
    } catch (PathNotFoundException e) {
      fail(e.getMessage());
    }

    try {
      assertFalse("The node shouldn't has mix:versionable", testRoot.getNode("file1")
                                                                    .isNodeType("mix:versionable"));
    } catch (PathNotFoundException e) {
      fail(e.getMessage());
    }
    ...
    try {
      assertEquals("Content must be equals",
                   testRoot.getProperty("file1[2]/jcr:content/any property").getString(),
                   "any content 1");
    } catch (PathNotFoundException e) {
      fail(e.getMessage());
    }
    ...
    // index 2 mixins
    try {
      assertFalse("The node shouldn't has mix:versionable", testRoot.getNode("file1[2]")
                                                                    .isNodeType("mix:versionable"));
    } catch (PathNotFoundException e) {
      fail(e.getMessage());
    }
