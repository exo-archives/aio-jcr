 ...
 * peter.nedonosko@exoplatform.com.ua 30.01.2008
 * 
...
public class JDBCStorageConnectionTest extends JcrImplBaseTest {
  ...
  public void setUp() throws Exception {
    ...
    dataManager = (WorkspacePersistentDataManager) session.getContainer().getComponentInstanceOfType(WorkspacePersistentDataManager.class); // .getTransientNodesManager().getTransactManager().getStorageDataManager()

    root = (NodeData) dataManager.getItemData(Constants.ROOT_UUID);

    TransientNodeData troot =
        TransientNodeData.createNodeData(root, InternalQName.parse("[]jdbcStorageConnectionTest"), Constants.NT_UNSTRUCTURED);
    ...
    testRoot = (NodeData) dataManager.getItemData(root, troot.getQPath().getEntries()[troot.getQPath().getEntries().length - 1]);
  ...
  protected void tearDown() throws Exception {
    ...
    for (PropertyData p: cps) {
      TransientPropertyData tp = 
        new TransientPropertyData(p.getQPath(), p.getIdentifier(), p.getPersistedVersion(), p.getType(), p.getParentIdentifier(), p.isMultiValued());
      chlog.add(ItemState.createDeletedState(tp));  
    }
    
    // just a TransientNodeData
    TransientNodeData troot =
      new TransientNodeData(testRoot.getQPath(), testRoot.getIdentifier(), testRoot.getPersistedVersion(), testRoot.getPrimaryTypeName(),
          testRoot.getMixinTypeNames(), testRoot.getOrderNumber(), testRoot.getParentIdentifier(), testRoot.getACL());
  ...
  public void testGetItem_InheritedACL() throws RepositoryException, IllegalNameException {
    ...
    NodeData troot = (NodeData) dataManager.getItemData(root,
        new QPathEntry(InternalQName.parse("[]jdbcStorageConnectionTest"), 1));
