...
public class JDBCStorageConnectionTest extends JcrImplBaseTest {
  ...
  public void setUp() throws Exception {
    ...
    dataManager = (WorkspacePersistentDataManager) session.getContainer()
    ...
    TransientNodeData troot = TransientNodeData.createNodeData(root,
                                                               InternalQName.parse("[]jdbcStorageConnectionTest"),
                                                               Constants.NT_UNSTRUCTURED);

    TransientPropertyData pt = TransientPropertyData.createPropertyData(troot,
    ...
    testRoot = (NodeData) dataManager.getItemData(root,
                                                  troot.getQPath().getEntries()[troot.getQPath()
                                                                                     .getEntries().length - 1]);
  ...
  protected void tearDown() throws Exception {
    ...
    for (PropertyData p : cps) {
      TransientPropertyData tp = new TransientPropertyData(p.getQPath(),
                                                           p.getIdentifier(),
                                                           p.getPersistedVersion(),
                                                           p.getType(),
                                                           p.getParentIdentifier(),
                                                           p.isMultiValued());
      chlog.add(ItemState.createDeletedState(tp));
    }

    // just a TransientNodeData
    TransientNodeData troot = new TransientNodeData(testRoot.getQPath(),
                                                    testRoot.getIdentifier(),
                                                    testRoot.getPersistedVersion(),
                                                    testRoot.getPrimaryTypeName(),
                                                    testRoot.getMixinTypeNames(),
                                                    testRoot.getOrderNumber(),
  ...
  public void testGetItem_InheritedACL() throws RepositoryException, IllegalNameException {
    ...
    NodeData troot = (NodeData) dataManager.getItemData(root,
                                                        new QPathEntry(InternalQName.parse("[]jdbcStorageConnectionTest"),
                                                                       1));

    assertEquals("Inherited acl should be here", root.getACL().getOwner(), troot.getACL()
                                                                                .getOwner());
  }
  ...
  public void testGetItem_MixOwneable() throws RepositoryException, IllegalNameException {
    ...
    // prepare mixin
    TransientPropertyData mixin = TransientPropertyData.createPropertyData(testRoot,
                                                                           Constants.JCR_MIXINTYPES,
                                                                           PropertyType.NAME,
                                                                           true,
                                                                           new TransientValueData(Constants.EXO_OWNEABLE));

    TransientPropertyData owner = TransientPropertyData.createPropertyData(testRoot,
                                                                           Constants.EXO_OWNER,
                                                                           PropertyType.STRING,
                                                                           false,
  ...
  public void testGetItem_MixPrivilegeable() throws RepositoryException, IllegalNameException {
    ...
    vd.add(new TransientValueData(SystemIdentity.ANY + AccessControlEntry.DELIMITER
        + PermissionType.READ));

    vd.add(new TransientValueData("managers" + AccessControlEntry.DELIMITER
        + PermissionType.SET_PROPERTY));

    vd.add(new TransientValueData("operators" + AccessControlEntry.DELIMITER
        + PermissionType.SET_PROPERTY));
    vd.add(new TransientValueData("operators" + AccessControlEntry.DELIMITER
        + PermissionType.ADD_NODE));

    TransientPropertyData permissions = TransientPropertyData.createPropertyData(testRoot,
  ...
  public void testGetItem_MixOwneableMixPrivilegeable() throws RepositoryException,
