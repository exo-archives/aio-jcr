...
public class AbstractCollisionTest extends AbstractImportTest {
  ...
  protected void importUuidCollisionTest(boolean isSystemView,
    ...
    if (testedBehavior != ImportUUIDBehavior.IMPORT_UUID_COLLISION_THROW) {
      imporRoot = session.getRootNode()
                         .getNode(ROOT_NODE_NAME)
                         .getNode(IMPORT_ROOT_NODE_NAME)
                         .getNode(EXPORT_ROOT_NODE_NAME);
    } else {
      imporRoot = session.getRootNode().getNode(ROOT_NODE_NAME).getNode(IMPORT_ROOT_NODE_NAME);
    }

    checkResult(imporRoot,
                session.getRootNode().getNode(ROOT_NODE_NAME).getNode(EXPORT_ROOT_NODE_NAME),
                result,
                isImportedByStream,
                testedBehavior);
    ...
    if ((testedBehavior == ImportUUIDBehavior.IMPORT_UUID_COLLISION_REMOVE_EXISTING || testedBehavior == ImportUUIDBehavior.IMPORT_UUID_COLLISION_REPLACE_EXISTING)
      ...
      for (ItemState state : changesLog.getAllStates()) {
        assertTrue(state.getAncestorToSave()
                        .equals(((NodeImpl) session.getRootNode().getNode(ROOT_NODE_NAME)).getData()
                                                                                          .getQPath()));
      }
    }
  ...
  private void checkFamilyTree(final Node checkRoot,
                               final Family family,
                               final String referenceableUuid) throws RepositoryException {
   ...
   * @throws SAXException
   */
  private void preformImport(boolean isSystemView,
                             boolean isExportedByStream,
                             boolean isImportedByStream,
                             XmlSaveType saveType,
                             int testedBehavior) throws RepositoryException,
  ...
  private Node prepareForImport(Node testRootNode) throws RepositoryException {
    ...
    exportRootNode.getNode(FAMILY_B_PARENT)
                  .getNode(FAMILY_B_FIRST_CHILD_NODE_NAME)
