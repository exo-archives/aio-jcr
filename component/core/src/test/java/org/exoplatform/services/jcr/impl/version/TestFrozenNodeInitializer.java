      ...
      for (ItemState result : testChanges) {
        ...
        if (resultData.getQPath().getName().equals(state.getData().getQPath().getName())) {
          if (resultData.getQPath().getName().equals(PROPERTY_IGNORED))
            fail("Ignored property can't be stored in frozen state: "
                + resultData.getQPath().getAsString());
          if (resultData.getQPath().getName().equals(NODE_IGNORED))
            fail("Ignored node can't be stored in frozen state: "
                + resultData.getQPath().getAsString());
          log.info("...found in frozen changes " + resultData.getQPath().getAsString());
          continue next;
        }
      }
      if (!(state.getData().getQPath().getName().equals(PROPERTY_IGNORED) || state.getData()
                                                                                  .getQPath()
                                                                                  .getName()
                                                                                  .equals(NODE_IGNORED)))
        fail("Change is not stored in frozen state: " + state.getData().getQPath().getAsString());
      ...
      for (ItemState result : testChanges) {
        ...
        if (resultData.getQPath().getName().equals(state.getData().getQPath().getName())) {
          if (resultData.getQPath().getName().equals(NODE_VERSIONED)) {
            ...
            PropertyData vh = (PropertyData) session.getTransientNodesManager()
                                                    .getItemData((NodeData) state.getData(),
                                                                 new QPathEntry(Constants.JCR_VERSIONHISTORY,
                                                                                0));

            String vhUuid = new String(vh.getValues().get(0).getAsByteArray());

            QPath childVersionHistoryPropertyPath = QPath.makeChildPath(resultData.getQPath(),
                                                                        Constants.JCR_CHILDVERSIONHISTORY);
            ItemState cvhState = changesLog.getItemState(childVersionHistoryPropertyPath);
            assertNotNull("Frozen state of node of type nt:versionedChild hasn't jcr:childVersionHistory property: "
                              + resultData.getQPath().getAsString(),
                          cvhState);

            String cvhUuid = new String(((PropertyData) cvhState.getData()).getValues()
                                                                           .get(0)
                                                                           .getAsByteArray());
            assertEquals("jcr:childVersionHistory property in frozen state contains wrong uuid",
                         vhUuid,
                         cvhUuid);
