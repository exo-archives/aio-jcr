...
public class TestRemapping extends JcrImplBaseTest {
  ...
  public void testRemappingSQL() throws Exception {
    ...
    QueryResult res = qManager.createQuery("SELECT * FROM nt:unstructured WHERE jcr:path LIKE '/"
                                               + ORIGINAL_PREFIX + ":" + TEST_NAME + "'",
                                           Query.SQL).execute();

    assertEquals(1, res.getNodes().getSize());
    res = qManager.createQuery("SELECT * FROM nt:unstructured WHERE jcr:path LIKE '/" + NEW_PREFIX
                                   + ":" + TEST_NAME + "'",
                               Query.SQL).execute();
    assertEquals(1, res.getNodes().getSize());
    ...
    Session sess = repository.login(cred, "ws");
    qManager = sess.getWorkspace().getQueryManager();
    res = qManager.createQuery("SELECT * FROM nt:unstructured WHERE jcr:path LIKE '/"
                                   + ORIGINAL_PREFIX + ":" + TEST_NAME + "'",
                               Query.SQL).execute();
