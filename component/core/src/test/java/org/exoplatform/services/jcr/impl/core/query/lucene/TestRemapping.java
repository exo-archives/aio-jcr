...
public class TestRemapping extends JcrImplBaseTest {
  ...
  public void setUp() throws Exception{
    ...
    for(int i=0; i<prefs.length; i++){
      if(prefs[i].equalsIgnoreCase(ORIGINAL_PREFIX)) isOriginal = true;
      if(prefs[i].equalsIgnoreCase(NEW_PREFIX)) isNew =true;
    }
  ...
  public void testRemappingXPath() throws Exception {
    ...
    QueryResult res = qManager.createQuery(ORIGINAL_PREFIX+":"+TEST_NAME, Query.XPATH)
        .execute();
...
public void testRemappingSQL() throws Exception {
    ...
    QueryResult res = qManager.createQuery("SELECT * FROM nt:unstructured WHERE jcr:path LIKE '/" +ORIGINAL_PREFIX+":"+TEST_NAME+"'", Query.SQL).execute();

    assertEquals(1, res.getNodes().getSize());
    res = qManager.createQuery("SELECT * FROM nt:unstructured WHERE jcr:path LIKE '/" +NEW_PREFIX+":"+TEST_NAME+"'", Query.SQL).execute();
    assertEquals(1, res.getNodes().getSize());
    ...
    Session sess = repository.login(cred, "ws");
    qManager = sess.getWorkspace().getQueryManager();
    res = qManager.createQuery("SELECT * FROM nt:unstructured WHERE jcr:path LIKE '/" +ORIGINAL_PREFIX+":"+TEST_NAME+"'", Query.SQL).execute();
