...
public class TestVersionableRemove extends BaseVersionTest {
  ...
  public void setUp() throws Exception {
    ...
    ws2Session.getWorkspace().clone(
        "ws1", 
        ws1Versionable.getPath(), 
        ws2TestRoot.getPath() + "/ws2Versionable", 
        true);
  ...
  private void prepareChildVersionHistory() throws Exception {
    ...
    ws2Session.getWorkspace().clone(
        "ws1", 
        ws1VersionableChild.getPath(), 
        ws2TestRoot.getPath() + "/versionableChild", 
        true);
  ...
  /**
   * Check if remove of versionable node that is contained in child version history 
   * will leave this child history because this history is contained in another version history.
   * 
   * @throws Exception
   */
  public void testRemoveChildVersionHistory() throws Exception {
    ...
    assertEquals("Corresponding nodes mut have same uuid", ws1VersionableChild.getUUID(), ws2VersionableChild.getUUID());
    assertEquals("Corresponding versionable mut have same history", ws1VersionableChild.getVersionHistory().getUUID(), ws2VersionableChild.getVersionHistory().getUUID());
