...
public class TestVersionableRemove extends BaseVersionTest {
  ...
  public void setUp() throws Exception {
    ...
    ws2Session.getWorkspace().clone("ws1",
  ...
  public void testRemoveChildVersionHistory() throws Exception {
    ...
    assertEquals("Corresponding nodes mut have same uuid",
                 ws1VersionableChild.getUUID(),
                 ws2VersionableChild.getUUID());
    assertEquals("Corresponding versionable mut have same history",
                 ws1VersionableChild.getVersionHistory().getUUID(),
                 ws2VersionableChild.getVersionHistory().getUUID());
    ...
    try {
      ...
      fail("A child version history must not exists after the parent history remove but found at "
          + vh.getPath());
    } catch (ItemNotFoundException e) {
      // ok
    }
