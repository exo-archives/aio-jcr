...
public class TestRandomValueIO extends JcrImplBaseTest {
  ...
  public void testUpdate_SmallToBigValue() throws Exception {
    ...
    if ((fmemAfter - fmem) >= (pos + testFile.length()))
      log.warn("Free memory must not be increased on value of the new Value size but does. Was "
          + fmem + " current " + fmemAfter);
    ...
    assertEquals("Value data length must be increased ",
                 pos + testFile.length(),
                 newexv.getLength());
  ...
  public void testAddLength_SmallToBigValue_Persistent() throws Exception {
    ...
    if ((tmemAfter - tmem) >= pos)
      log.warn("JVM total memory should not be increased on size of the new Value but does. Was "
          + tmem + " current " + tmemAfter);
  ...
  public void testUpdateSmallValues() throws Exception {
    ...
    try {
      ...
      // the value obtained by getXXX must be same as on setProperty()
      compareStream(new ByteArrayInputStream(new byte[] {}), testRoot.getProperty(pname)
                                                                     .getStream());
      p.setValue(exv);
