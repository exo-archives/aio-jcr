...
public class TestSessionDataManager extends JcrImplBaseTest {
  ...
  public void setUp() throws Exception {
    ...
    //testRoot = (NodeImpl)root.createChildNode(TEST_ROOT, "nt:unstructured", true, true);
    TransientNodeData data = TransientNodeData.createNodeData(
        root.nodeData(),
...
//    log.info(" >after commit> "+modificationManager.dump());
    ...
    TransientNodeData data = TransientNodeData.createNodeData(
        parent,
        new InternalQName(null,"testItemReferencePool1"), 
        new InternalQName(Constants.NS_NT_URI, "unstructured"));
    ...
    // add one more node 
    data = TransientNodeData.createNodeData(
        parent,
  ...
  public void testSessionChangesLog() throws Exception {
    ...
    TransientNodeData data = TransientNodeData.createNodeData(
        parent,
        new InternalQName(null,"testSessionChangesLogN1"), 
        new InternalQName(Constants.NS_NT_URI, "unstructured"));
...
//    System.out.println(" > !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! after testSessionChangesLog> "+modificationManager.dump());

    // delete this node ... state should be DELETED
    modificationManager.delete(data);
    assertEquals(2, changesLog.getAllStates().size());
    List <ItemState> lst = changesLog.getItemStates(node1.getInternalUUID());
    assertEquals(2, lst.size());
    assertEquals(ItemState.DELETED, changesLog.getItemState(node1.getInternalUUID()).getState());

//    System.out.println(" > 2 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! after testSessionChangesLog> "+modificationManager.dump());
    ...
    assertEquals(3, changesLog.getItemStates(node1.getInternalUUID()).size());
    assertEquals(ItemState.ADDED, changesLog.getItemState(node1.getInternalUUID()).getState());
    
    assertEquals(3, changesLog.getAllStates().size());
    
//    System.out.println(" > 3 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! after testSessionChangesLog> "+modificationManager.dump());

    // ... add property to the node1
    TransientPropertyData prop = TransientPropertyData.createPropertyData(
      (NodeData)node1.getData(), 
      new InternalQName(null,"testSessionChangesLogP1"), 
      PropertyType.STRING, false);
...
//    // 4 changes: 3 for node1, and 1- for prop1
  ...
  public void testReadMethods() throws Exception {
    ...
    NodeData parent = (NodeData)testRoot.getData(); 
    TransientNodeData someData = TransientNodeData.createNodeData(
        parent,
        new InternalQName(null,"testReadMethodsN1"), 
        new InternalQName(Constants.NS_NT_URI, "unstructured"));
    //NodeImpl node1 = (NodeImpl)modificationManager.update(ItemState.createAddedState(data), true);
    ...
    TransientNodeData data = TransientNodeData.createNodeData(
        parent,
        new InternalQName(null,"testReadMethodsN2"), 
        new InternalQName(Constants.NS_NT_URI, "unstructured"));
    NodeImpl node2 = (NodeImpl)modificationManager.update(ItemState.createAddedState(data), true);
...
//    List <PropertyImpl> props = modificationManager.getChildProperties(parent, true);
...
//    Collections.copy(dest, src);
  ...
  public void testCommitAndRefresh() throws Exception {
    ...
    NodeData parent = (NodeData)testRoot.getData(); 
    TransientNodeData data1 = TransientNodeData.createNodeData(
        parent,
        new InternalQName(null,"testCommitAndRefreshN1"), 
        new InternalQName(Constants.NS_NT_URI, "unstructured"));
