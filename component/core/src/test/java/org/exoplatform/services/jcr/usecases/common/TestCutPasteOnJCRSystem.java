...
public class TestCutPasteOnJCRSystem extends BaseStandaloneTest {  
  ...
  private String workspaceName ;
    ...
    initTaxonomyTree() ;
     /* 
      * So, we have exo:taxonomies tree like that:
      * jcr:system/exo:taxonomies(nt:untructured)
      *                          /cms
      *                              /news
      *                              /sport
      *                              /calendar 
...
System.out.println("Move from "+cutPath+"to "+pastedPath);
    ...
    Session otherAdminSession = repository.login(new SimpleCredentials("admin", "admin".toCharArray()), workspaceName);
...
System.out.println("Move from "+newsSrcPath+" to "+destPath2Paste+" "+newsSrcPath.substring(newsSrcPath.lastIndexOf("/")));
...
System.out.println("Move from "+srcPath+" to "+destPath);
  ...
  /* this is NodeType config for exo:taxonomy  
