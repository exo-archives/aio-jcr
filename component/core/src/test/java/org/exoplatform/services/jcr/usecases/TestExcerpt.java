...
public class TestExcerpt extends BaseUsecasesTest {
  ...
  public void testExcerpt() throws Exception {
    ...
    Query q1 = queryManager.createQuery("select excerpt(.) from exo:article where contains(., 'test')",
                                        Query.SQL);
    ...
    System.out.println("\n\n-----Test with XPATH");
    Query q2 = queryManager.createQuery("//*[jcr:contains(., 'test')]/(@exo:text|rep:excerpt(.))",
                                        Query.XPATH);
