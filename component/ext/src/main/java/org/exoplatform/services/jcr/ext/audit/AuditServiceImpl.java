...
public class AuditServiceImpl implements AuditService {
  ...
  public void addRecord(Item item, int eventType) throws RepositoryException {
    ...
    if (vancestor != null) {
      // auditale node under a version control, set related properties to the audit record
  ...
  private class AuditSession {
    ...
    private NodeData getAuditHistoryNodeData() throws RepositoryException {
      ...
      PropertyData pData =
          (PropertyData) dataManager.getItemData((NodeData) ((NodeImpl) node).getData(),
                                                 new QPathEntry(AuditService.EXO_AUDITHISTORY, 0));
      if (pData != null)
        try {
    ...
    private NodeData getAuditStorage() throws RepositoryException {
      ...
      if (storage == null) {
        ...
        TransientNodeData exoAuditNode =
            TransientNodeData.createNodeData((NodeData) ((NodeImpl)(session.getRootNode())).getData(),
