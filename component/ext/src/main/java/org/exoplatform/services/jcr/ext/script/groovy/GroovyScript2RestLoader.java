...
public class GroovyScript2RestLoader implements Startable {
  ...
  /**
   * See {@link SessionProviderService}, {@link ThreadLocalSessionProviderService}.
   */
  ...
  /**
   * @param binder
   *          binder for RESTful services
   * @param groovyScriptInstantiator
   *          instantiate groovy scripts
   * @param repositoryService
   *          See {@link RepositoryService}
   * @param sessionProviderService
   *          See {@link SessionProviderService}
   * @param configurationManager
   *          for solve resource loading issue in common way
   * @param handler
   * @param params
   ...
   * 
   * @param url
   *          the URL. The <code>url.toString()</code> must be corresponded to script class.
  ...
  /**
   * Get node type for store scripts, may throw {@link IllegalStateException} if <tt>nodeType</tt>
   * not initialized yet.
   * 
   ...
   * @param stream
   *          the stream which represents groovy script.
   * @throws IOException
   ...
   *          the key which must be corresponded to object class name.
   * @param name
   *          this name will be passed to compiler to get understandable if compilation failed
   * @param stream
   *          the stream which represents groovy script.
   * @throws IOException
   *           if script can't be loaded or parsed.
  ...
  /**
   * See {@link Handler}. Not used in this class but should be in constructor parameters for correct
   * order of start components.
   */
   ...
   * @param stream
   *          data stream for property jcr:data
  ...
  /**
   * This method is useful for clients that can send script in request body without form-data. At
   * required to set specific Content-type header 'script/groovy'.
   * 
   * @param stream
   *          the stream that contains groovy source code
   * @param uriInfo
   *          see {@link UriInfo}
   * @param repository
   *          repository name
   ...
   *          workspace name
   * @param path
   *          path to resource to be created
   ...
   * NOTE even we use iterator item should be only one, rule one address - one script. This method
   * is created just for comfort loading script from HTML form. NOT use this script for uploading
   * few files in body of 'multipart/form-data' or other type of multipart.
   * 
   * @param items
   *          iterator {@link FileItem}
   * @param uriInfo
   *          see {@link UriInfo}
   * @param repository
   *          repository name
   ...
   *          workspace name
   * @param path
   *          JCR path to node that contains script
  ...
  /**
   * Change exo:autoload property. If this property is 'true' script will be deployed automatically
   * when JCR repository startup and automatically re-deployed when script source code changed.
   * 
   * @param repository
   *          repository name
   * @param workspace
   ...
   *          JCR path to node that contains script
   * @param state
   *          value for property exo:autoload, if it is not specified then 'true' will be used as
   *          default. <br />
  ...
  /**
   * Deploy groovy script as REST service. If this property set to 'true' then script will be
   * deployed as REST service if 'false' the script will be undeployed. NOTE is script already
   * deployed and <tt>state</tt> is <tt>true</tt> script will be re-deployed.
   * 
   * @param repository
   *          repository name
   * @param workspace
   *          workspace name
   * @param path
   *          the path to JCR node that contains groovy script to be deployed
   */
  @POST
    ...
    try {
      ...
      if (state) {
        ...
        if (!loadScript(unifiedNodePath, path, script.getProperty("jcr:data").getStream())) {
          String message = "Can't bind script "
              + path
   ...
   *          Workspace name.
   * @param name
   *          Additional search parameter. If not emtpy method returns the list of script names
   *          matching wildcard else returns all the scripts found in workspace.
   * @return
   */
  @POST
  @Produces(MediaType.APPLICATION_JSON)
