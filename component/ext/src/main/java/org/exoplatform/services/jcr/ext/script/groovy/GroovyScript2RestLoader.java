...
public class GroovyScript2RestLoader implements Startable {
  ...
  public GroovyScript2RestLoader(ResourceBinder binder,
    ...
    if (params != null) {
      nodeType = params.getValuesParam("nodetype") != null ? params
          .getValueParam("nodetype").getValue() : DEFAULT_NODETYPE;

      ObjectParameter param = params.getObjectParam("observation.config");
      if (param != null)
        observationListenerConfiguration = (ObservationListenerConfiguration) param
            .getObject();
    }
  }
   ...
   */
  public void loadScript(URL url) throws InvalidResourceDescriptorException,
      IOException {

    ResourceContainer resourceContainer = (ResourceContainer) groovyScriptInstantiator
        .instantiateScript(url);
    binder.bind(resourceContainer);

    // add mapping script URL to name of class.
    scriptsURL2ClassName.put(url.toString(), resourceContainer.getClass()
        .getName());
   ...
   */
  public void loadScript(String key, InputStream stream)
      throws InvalidResourceDescriptorException, IOException {
    ResourceContainer resourceContainer = (ResourceContainer) groovyScriptInstantiator
        .instantiateScript(stream);
    binder.bind(resourceContainer);
  ...
  public void start() {
    ...
    try {
      ...
      List<String> workspaceNames = observationListenerConfiguration
          .getWorkspaces();
...
//        Session session = sessionProvider.getSession(workspaceName, repository);
        ...
        session.getWorkspace().getObservationManager().addEventListener(
            ...
            true, null, new String[]{ nodeType }, false);
