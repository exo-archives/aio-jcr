...
public class ConcurrentModificationCheckerTest extends BaseTestCaseChecker {
  ...
  public void testTwoThreads() throws Exception {
    String relPath[] = new String[] {createRelPath(getRandomLong()),
        createRelPath(getRandomLong()), createRelPath(getRandomLong())};
    
    String fileName[] = new String[] {"nt_file_" + getRandomLong(), 
        "nt_file_" + getRandomLong(), "nt_file_" + getRandomLong()};
    ...
    for (int i = 0; i < 2; i++) {
    {
      String url = "http://" + getMaxPriorityMember().getIpAddress() + ":" 
                             + getMaxPriorityMember().getPort()  
                             + ReplicationTestService.Constants.BASE_URL
                             + "/" + workingRepository
                             + "/" + workingWorkspace
                             + "/" + getMaxPriorityMember().getLogin()
    ...
    
    {
      String url = "http://" + getMinPriorityMember().getIpAddress() + ":" 
                             + getMinPriorityMember().getPort()  
                             + ReplicationTestService.Constants.BASE_URL
                             + "/" + workingRepository
                             + "/" + workingWorkspace
                             + "/" + getMinPriorityMember().getLogin()
    ...
    for(MemberInfo memberInfo : getAllMembers()) {
     ...
     //compare with srcRelPath2
      {
        String url = "http://" + memberInfo.getIpAddress() + ":" 
                               + memberInfo.getPort()  
                               + ReplicationTestService.Constants.BASE_URL
                               + "/" + workingRepository
                               + "/" + workingWorkspace
