...
public class LockCommand extends WebDavCommand {
  ...
  public Response getProperties(
    ...
    try {      
      ...
      try {        
        ...
        lockDiscovery.read(webDavService, node);        
        lockDiscovery.setLockToken(lockResult.getLockToken());
        
        return createLockResponse(lockDiscovery);        
      } catch (PathNotFoundException pexc) {
        String fullPath = repoName + "/" + repoPath;
        return doFakeLock(fullPath, (LockInfoRepresentation)requestRepresentation);
      }
    } catch (Exception exc) {
  ...
  private Response createLockResponse(LockDiscoveryRepresentation lockDiscovery) {
    SinglePropertyRepresentation propertyRepresentation = new SinglePropertyRepresentation(lockDiscovery); 
    
    ByteArrayOutputStream outStream = new ByteArrayOutputStream();
    
    XmlResponseWriter writer = new XmlResponseWriter(propertyRepresentation);
  ...
  private Response doFakeLock(String resourceHref, LockInfoRepresentation lockInfoRepresentation) throws Exception {
