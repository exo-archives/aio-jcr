...
public class CheckOutCommand extends WebDavCommand {
  ...
  private static Log log = ExoLogger.getLogger("jcr.CheckOutCommand");
  
  public CheckOutCommand(WebDavService webDavService, 
      ResourceDispatcher resourceDispatcher,
      ThreadLocalSessionProviderService sessionProviderService) {
    super(webDavService, resourceDispatcher, sessionProviderService);
  }
  
  @HTTPMethod(WebDavMethod.CHECKOUT)
  ...
  public Response checkout(
    ...
    try {
      ...
      if (!node.isNodeType(DavConst.NodeTypes.MIX_VERSIONABLE)) {
        throw new AccessDeniedException("Can't check-out not versionable node!");
      }
