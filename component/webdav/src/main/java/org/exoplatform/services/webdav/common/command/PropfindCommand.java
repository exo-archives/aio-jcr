...
public class PropfindCommand extends WebDavCommand {
  ...
  protected boolean process() throws RepositoryException {
    ...
    WebDavResource davResource = getResourceFactory().getSrcResource(false);
    
    int depth = davRequest().getDepth();
    
    ResponseBuilder builder = new ResponseBuilder(davResource, (PropFindDocument)reqDoc);
    ArrayList<Response> responses = builder.getResponses(depth);
