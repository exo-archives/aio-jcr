...
public class WebDavConfigImpl implements WebDavConfig {
  ...
  public static final String INIT_PARAM_CATALOG_NAME = "catalog-name";
  ...
  public WebDavConfigImpl(InitParams params, String configFilePath) throws Exception {
    ...
    ValueParam pCatalogName = params.getValueParam(INIT_PARAM_CATALOG_NAME);
    if (pCatalogName != null) {
      _catalogName = pCatalogName.getValue();
      log.info(INIT_PARAM_CATALOG_NAME + " = " + _catalogName);
    }

    ValueParam pRepositoryName = params.getValueParam(INIT_PARAM_REPOSITORY_NAME);
    if (pRepositoryName != null) {
      _repositoryName = pRepositoryName.getValue();
      log.info(INIT_PARAM_REPOSITORY_NAME + " = " + _repositoryName);
    }
...
//    Node requestDocuments = DavUtil.getChildNode(configNode, XML_REQUESTDOCUMENTS);
//    
//    NodeList documentNodes = requestDocuments.getChildNodes();
//    for (int i = 0; i < documentNodes.getLength(); i++) {
//      Node documentNode = documentNodes.item(i);
//      parseRequestDocument(documentNode);
//    }
