...
public abstract class GenericResource implements Resource {
	...
	public Set <HierarchicalProperty> getProperties(boolean namesOnly) throws RepositoryException {
		...
		props.add(namesOnly ? new HierarchicalProperty(DISPLAYNAME) : getProperty(DISPLAYNAME));
		props.add(namesOnly ? new HierarchicalProperty(RESOURCETYPE) : getProperty(RESOURCETYPE));
		props.add(namesOnly ? new HierarchicalProperty(CREATIONDATE) : getProperty(CREATIONDATE));
		return props;
	}
  ...
  public static HierarchicalProperty lockDiscovery(String token, String lockOwner) {    
