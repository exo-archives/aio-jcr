...
public class TestLock extends LockTest {
  ...
  public void testSimpleLock() throws Exception {
    ...
    Response restResponse = new LockCommand(lockHolder).lock(session, path, body, new Depth(null), "" + Integer.MAX_VALUE);
    ...
    restResponse = new PropFindCommand().propfind(session, path, body, Integer.MAX_VALUE, "http://localhost");
    ...
    HierarchicalProperty multistatus = 
      (HierarchicalProperty)transformer.readFrom(new ByteArrayInputStream(outputStream.toByteArray()));
