...
public class TestUnLock extends LockTest {
  ...
  public void testUnLock() throws Exception {
    ...
    assertEquals(WebDavStatus.NO_CONTENT, new UnLockCommand(lockHolder).unLock(session, path, null)
                                                                       .getStatus());

    assertEquals(WebDavStatus.OK, new LockCommand(lockHolder).lock(session,
                                                                   path,
                                                                   null,
                                                                   new Depth(null),
                                                                   "Infinity").getStatus());
    ...
    assertEquals(WebDavStatus.LOCKED, new UnLockCommand(lockHolder).unLock(session, path, null)
                                                                   .getStatus());
    ...
    assertEquals(WebDavStatus.NO_CONTENT, new UnLockCommand(lockHolder).unLock(session,
                                                                               path,
                                                                               tokens).getStatus());
  }
  ...
  public void testNullResourceUnLock() throws Exception {
    ...
    assertEquals(WebDavStatus.NOT_FOUND, new UnLockCommand(lockHolder).unLock(session, path, null)
                                                                      .getStatus());

    Response response = new LockCommand(lockHolder).lock(session,
                                                         path,
                                                         null,
                                                         new Depth(null),
                                                         "Infinity");
