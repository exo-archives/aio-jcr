...
import org.exoplatform.services.jcr.rmi.api.observation.ServerEventListenerProxy;
...
public class ServerObservationManager extends ServerObject implements RemoteObservationManager {
   ...
   * 
   */
  private static final long                                      serialVersionUID = 1457088328353230733L;
  ...
  /**
   * The queue to which event listener proxies post events to be reported by the
   * {@link #getNextEvent(long)} method.
   */
  private org.exoplatform.services.jcr.rmi.api.observation.Queue queue;
  ...
  public void addEventListener(long listenerId,
    ...
    synchronized (this) {
      ...
      if (proxy == null) {
        proxy = new ServerEventListenerProxy(getFactory(), listenerId, getQueue());
