...
import org.exoplatform.services.jcr.rmi.api.xml.SessionImportContentHandler;
...
public class ClientSession extends ClientObject implements Session, NamespaceAccessor {
  ...
  public Workspace getWorkspace() {
    if (workspace == null) {
      try {
        workspace = getFactory().getWorkspace(this, remote.getWorkspace());
      } catch (RemoteException ex) {
        throw new RemoteRuntimeException(ex);
      }
    }
  ...
  /**
   * Returns the {@link SerialValueFactory#getInstance()}. {@inheritDoc}
   */
  ...
  public String[] getAllNamespacePrefixes() throws RepositoryException {
    try {
      return remote.getAllNamespacePrefixes();
    } catch (RemoteException e) {
  ...
  public String getNamespacePrefixByURI(String uri) throws NamespaceException, RepositoryException {
    try {
      return remote.getNamespacePrefixByURI(uri);
    } catch (RemoteException e) {
      throw new RepositoryException(e);
    }
  }
