...
/**
 * Stateful value implementation. This class implements the Context part of the
 * State design pattern (GoF) used for managing the JCR Value states.
 * <p>
...
final class SerialValue implements ExtendedValue, Serializable {
  ...
  /**
   * Type of the underlying value. Note that this type is never changed even if
   * the value state changes. Thus the type is memorized in this member variable
   * when the value instance is created.
   */
  private int           type;

  /**
   * The underlying value instance. This is the state reference that changes
  ...
  /**
   * Creates a generic value instance. A new InitialValue instance that wraps
   * the given static value instance is created to intercept the first getter
   * method. The created InitialValue instance has the responsibility of
   * changing the value state.
   * 
   ...
   * depends on the current state of the value, see the JCR documentation for
   * the details.
   * 
   * @return binary value
  ...
  /**
   * Returns the long representation of the value. The actual behaviour depends
   * on the current state of the value, see the JCR documentation for the
   * details.
   * 
   * @return long value
  ...
  /**
   * Returns the date representation of the value. The actual behaviour depends
   * on the current state of the value, see the JCR documentation for the
   * details.
   * 
   * @return date value
  ...
  /**
   * Returns the value type. Note that value type remains the same even if the
   * underlying value is converted to another type during the stream/non-stream
   * state transition.
   * 
   * @return value type
