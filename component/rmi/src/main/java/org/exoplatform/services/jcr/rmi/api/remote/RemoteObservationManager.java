 ...
 * {@link org.exoplatform.services.jcr.rmi.impl.server.ServerObservationManager
...
public interface RemoteObservationManager extends Remote {
   ...
   * {@link javax.jcr.observation.ObservationManager#addEventListener(javax.jcr.observation.EventListener, int, String, boolean, String[], String[], boolean)
   * ObservationManager.addEventListener()} method. See class comment for an explanation on how the
   * <code>listenerId</code> is used.
   * 
   * @param listenerId
   *          The identification of the listener on the client side to which events will be
   *          directed.
   * @param eventTypes
   *          The mask of event types to be sent to this listener.
   * @param absPath
   *          The root item defining a subtree for which events are to be delivered.
   * @param isDeep
   *          <code>true</code> if the events from the complete subtree rooted at
   *          <code>absPath</code> are to be sent or only for the item at the given path.
   * @param uuid
   *          An optional list of node UUIDs for which events are to be sent. If <code>null</code>
   *          this parameter is ignored.
   * @param nodeTypeName
   *          An optional list of node type names for which events are to be sent. If
   *          <code>null</code> this parameter is ignored.
   * @param noLocal
   *          <code>true</code> if only events are to be sent which do not originate from the
  ...
  /**
   * Remote version of the
   * {@link javax.jcr.observation.ObservationManager#removeEventListener(javax.jcr.observation.EventListener)
   * ObservationManager.removeEventListener()} method. See class comment for an explanation on how
   * the <code>listenerId</code> is used.
   * 
   * @param listenerId
   *          The identification of the listener on the client side to which events will be
   *          directed.
   ...
   * 
   * @param timeout
   *          The time in milliseconds to wait for the next event available to be dispatched. If
   *          negative or zero, this method waits for ever.
   * @return The {@link RemoteEventCollection} to be dispatched. <code>null</code> is returned if
