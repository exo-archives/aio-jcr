...
public class TestUpload extends TestCase {
  ...
  public void testRunUpload() {
    ...
    // -repo="db1" -ws="ws" \
    // -readtree="false"
    String[] params = new String[] { "-conf=bin\\config\\devel\\configuration-oraee1.xml",
        "-root=/testStorage/root1", "-tree=10-100-100-100", "-vdfile=bin\\img.tif", "-repo=db1",
        "-ws=ws", "-readtree=false" };
  ...
  public void _testRunRead() {
    ...
    // -repo="db1" -ws="ws" \
    // -readdc -read
    String[] params = new String[] { "-conf=bin\\config\\devel\\configuration-oraee.xml",
        "-root=/testStorage/root1", "-tree=10-100-100-100", "-vdfile=bin\\img.tif", "-repo=db1",
        "-ws=ws", "-read", "-readdc" };
    RepositoryDataUploader.main(params);
  }
