...
namespace exo_jcr.msofficeplugin.ppoint
{
    public partial class MainControl : UserControl
    {
        ...
        public void saveClick(_Application app, String entered_filename)
        {
            ...
            String localFilePath = app.ActivePresentation.FullName;
            

            if (localFilePath.StartsWith(connect.getCacheFolder()))
            {

                if (doc.Saved.Equals(MsoTriState.msoTrue))
                {
                }
                else
                {
            ...
            else
            {
                ...
                String localNameToSave = (localFolder + entered_filename + ".ppt").Replace('/', '\\');

                try
                {
                    String wFileName = localNameToSave;
                    object omissing = Missing.Value;
                    doc.SaveAs(wFileName, PpSaveAsFileType.ppSaveAsPresentation, MsoTriState.msoTrue);
                    

                    
                    String filename = app.ActivePresentation.Name;
                    String remoteFileName = path.Substring(path.IndexOf("/" + connect.Workspace));
        ...
        public int getFileList(String path)
        {
            try
            {
                ...
                if (status == DavStatus.MULTISTATUS)
                {
                   ...
                   // MessageBox.Show("SETTING PATH:"+path);
