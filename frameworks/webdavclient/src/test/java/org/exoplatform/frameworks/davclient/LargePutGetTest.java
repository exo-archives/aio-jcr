...
public class LargePutGetTest extends TestCase {
  ...
  public void testLargePutGet() throws Exception {
    ...
    ServerLocation serverLocation = TestConst.getTestServerLocation();

    DavOptions davOptions = new DavOptions(serverLocation);
    assertEquals(Const.HttpStatus.OK, davOptions.execute());
    
    DavPropFind davPropFind = new DavPropFind(serverLocation);
    davPropFind.setResourcePath(TEST_WORKSPACE);
    assertEquals(Const.HttpStatus.MULTISTATUS, davPropFind.execute());
    
    DavMkCol davMkCol = new DavMkCol(serverLocation);
    davMkCol.setResourcePath(TEST_FOLDER);
    assertEquals(Const.HttpStatus.CREATED, davMkCol.execute());
    
    davPropFind = new DavPropFind(serverLocation);
    ...
    
    {
      ...
      DavPut davPut = new DavPut(serverLocation);
    ...
    {
      DavGet davGet = new DavGet(serverLocation);
    ...
    {
      DavDelete davDelete = new DavDelete(serverLocation);
      davDelete.setResourcePath(TEST_FILE);
     
      assertEquals(Const.HttpStatus.NOCONTENT, davDelete.execute());
      
      davDelete = new DavDelete(serverLocation);
