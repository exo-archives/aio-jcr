 ...
 * the command to output nt:resource to Servlet Response
 * gets DefaultKeys.PATH attrribute from the Context and acts as follow: 
 * - if there is nt:resource Node on DefaultKeys.PATH displays it
 * - otherwise recursively tries to get nt:resource from incoming node's primary
 * items and display it
 * - throws PathNotFoundException if no such a node found  
 * WARNING: this mechanizm is not suitable for big files streaming as uses byte arry buffer for data transfer!   
    ...
    try {
      ...
      if(file.isNodeType("exo:article")) {
        try {
          ...
          throw e; //new ItemNotFoundException("No nt:resource node found at "+file.getPath()+" nor primary items of nt:resource type ");
        }
      } else {
        throw e; //new ItemNotFoundException("No nt:resource node found at "+file.getPath()+" nor primary items of nt:resource type ");
      }
...
//      throw new Exception("Invalid node type, expected nt:file or exo:article, have "+file.getPrimaryNodeType().getName()+" at "+file.getPath());
    ...
    String encoding = content.hasProperty("jcr:encoding")?
