 ...
 * @description mvn exo:pc -Ddeploy=param for deploy PC standalone
 */
...
//    project.getArtifact().setFile(earFile);
  ...
  private void execDeploy() throws Exception {
    ...
    if (deploy.equals("tomcat")) {
      ...
      Utils.printMessage("info","The task 'exo:pc -Ddeploy=" + deploy+ "' is invalid !\n");
      printInfo();
    }
  }
...
//    Utils.deployProject(outDir, outDir, project, false, ignoreProjects);
  ...
  private void printInfo() throws MojoExecutionException {
    String info =  
      "The 'exo:pc' maven2 plugin is used to assemble many exo modules into an application.\n" +
