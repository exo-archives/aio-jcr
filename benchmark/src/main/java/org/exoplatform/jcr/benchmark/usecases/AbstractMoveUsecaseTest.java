 ...
 * @version $Id$
 */
public abstract class AbstractMoveUsecaseTest extends JCRTestBase {
  
  protected static final int START_YEAR = 1984;
  ...
  public void doPrepare(TestCase tc, JCRTestContext context) throws Exception {
    ...
    for (int i=0; i<runIterations; i++) {
      ...
      for (int n=0; n<nodesCount; n++) {
        ...
        fileDate.set(Calendar.YEAR, START_YEAR); //  + rndYear.nextInt(2)
        fileDate.set(Calendar.MONTH, rndMonth.nextInt(1));
        fileDate.set(Calendar.DAY_OF_MONTH, 1 + rndDay.nextInt(4));
        ...
        //fres.setProperty("jcr:data", new ByteArrayInputStream(("the usecase content " + i + "-" + n).getBytes()));
        ...
        if (addMeatadata) {
          ...
          file.setProperty("dc:date", new Value[] {rootNode.getSession().getValueFactory().createValue(fileDate)});
