...
/**
 * <p>
 * TCP/IP NetBIOS host announcer implementation. Periodically broadcasts a host
 * announcement datagram to inform other Windows networking hosts of the local
 * hosts existence and capabilities.
 */
public class TcpipNetBIOSHostAnnouncer extends HostAnnouncer {
	 ...
	 *            Port to use
	 */
	public TcpipNetBIOSHostAnnouncer(String name, String domain, int intval,
			int port) {
	...
	protected void initialize() throws Exception {
		...
		if (m_bcastAddr == null)
			m_bcastAddr = InetAddress.getByName(NetworkSettings
	...
	protected void sendAnnouncement(String hostName, byte[] buf, int offset,
		...
		if (logger.isDebugEnabled())
			logger.info/* debug */("Send NetBIOS host announcement to "
		...
		m_nbdgram.SendDatagram(NetBIOSDatagram.DIRECT_GROUP, hostName,
				NetBIOSName.FileServer, getDomain(), NetBIOSName.MasterBrowser,
				buf, len, offset, m_bcastAddr, getPort());
	}
