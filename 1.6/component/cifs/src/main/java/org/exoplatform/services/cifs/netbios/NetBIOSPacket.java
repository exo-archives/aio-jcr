...
public class NetBIOSPacket {
	 ...
	 * 
	 * @param sessPkt
	 *            True if this is a NetBIOS session packet, else false.
	 */
	public void DumpPacket(boolean sessPkt) {
		...
		if (sessPkt == true) {
			...
			case RFCNetBIOSProtocol.SESSION_REQUEST:
				...
				logger.debug("Session request from "
	 ...
	 * 
	 * @param pktoff
	 *            Packet offset to pack the resource record.
	 * @param offset
	 *            Offset to name.
	 * @param qtyp
	 *            int
	 ...
	 * @return int
	 */
	public final int buildAddNameRequest(NetBIOSName name, int addrIdx,
			int tranId) {
	 ...
	 * @return int
	 */
	public final int buildRefreshNameRequest(NetBIOSName name, int addrIdx,
			int tranId) {
	 ...
	 * @return int
	 */
	public final int buildDeleteNameRequest(NetBIOSName name, int addrIdx,
			int tranId) {
	...
	public final int buildNameQueryRequest(NetBIOSName name, int tranId) {
		...
		setFlags(NetBIOSSession.hasWINSServer() ? 0
				: NetBIOSPacket.FLG_BROADCAST);
		setQuestionCount(1);
		return setQuestionName(name, NetBIOSPacket.NAME_TYPE_NB,
	 ...
	 * @return int
	 */
	public final int buildSessionSetupRequest(NetBIOSName fromName,
			NetBIOSName toName) {
		...
		DataPacker.putShort((short) (pos - RFCNetBIOSProtocol.HEADER_LEN),
				m_nbbuf, 2);
