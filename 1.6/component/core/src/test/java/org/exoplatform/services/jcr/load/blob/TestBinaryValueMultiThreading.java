...
public class TestBinaryValueMultiThreading extends JcrAPIBaseTest {
  ...
  protected class TestJCRClient extends Thread {
	...
	public void run() {
      ...
      Node testRemoteBigFiles = clientSession.getRootNode().getNode("testRemoteBigFiles");
      Node testUrlSmallFiles = clientSession.getRootNode().getNode("testUrlSmallFiles");
      Node testUrlBigFiles = clientSession.getRootNode().getNode("testUrlBigFiles");*/
      
	    for (int i = 0; i < FILES_COUNT; i++) {
 	 	    //check streams  
	      
        compareStream(new FileInputStream(LOCAL_SMALL_FILE), 
	   	    testLocalSmallFiles.getProperty("smallFile" + i + "/jcr:content/jcr:data").getStream());
        compareStream(new FileInputStream(LOCAL_BIG_FILE), 
          testLocalBigFiles.getProperty("bigFile" + i + "/jcr:content/jcr:data").getStream());
        
        /*compareStream(new FileInputStream(REMOTE_SMALL_FILE), 
          testRemoteSmallFiles.getProperty("smallFile" + i + "/jcr:content/jcr:data").getStream());
        compareStream(new FileInputStream(REMOTE_BIG_FILE), 
          testRemoteBigFiles.getProperty("bigFile" + i + "/jcr:content/jcr:data").getStream());
        compareStream(new URL(URL_SMALL_FILE).openStream(), 
          testUrlSmallFiles.getProperty("smallFile" + i + "/jcr:content/jcr:data").getStream());
        compareStream(new URL(URL_BIG_FILE).openStream(),
          testUrlBigFiles.getProperty("bigFile" + i + "/jcr:content/jcr:data").getStream());*/
  ...
  /**
   * The test must be runed in preconfigured environment:
   * LOCAL_SMALL_FILE, etc vars 
   */
  public void testMultiThreadReading() throws Exception {
    ...
    //Remote small files
    /*Node testRemoteSmallFiles = root.addNode("testRemoteSmallFiles");
    for (int i = 0; i < FILES_COUNT; i++) {
    ...
    //Remote big files
    Node testRemoteBigFiles = root.addNode("testRemoteBigFiles");
    for (int i = 0; i < FILES_COUNT; i++) {
    ...
    //Url small files
    Node testUrlSmallFiles = root.addNode("testUrlSmallFiles");
    for (int i = 0; i < FILES_COUNT; i++) {
    ...
    //Url big files
    Node testUrlBigFiles = root.addNode("testUrlBigFiles");
    for (int i = 0; i < FILES_COUNT; i++) {
